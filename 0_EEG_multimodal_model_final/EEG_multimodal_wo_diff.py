...
if __name__ == '__main__':
    ...
    # 10) Evaluate & Save
    # 1) 세그먼트 별 확률
    # 2) 완전 neutral(=표정 변화 거의 없음)로 넣는 버전
    
    ########여기 부분을 수정하면 apex 점수 반영 방법을 바꿈########
    #### apex 점수 미반영 #######################
    N_test = Xe_te.shape[0]
    fake_apex = np.zeros((N_test, 4), dtype=np.float32)
    fake_apex[:, -1] = 1.0 
    seg_probs = model.predict(
        [Xe_te, Xf_te, Xm_te, Xr_te, Xq_te, fake_apex],#Xa_te],
        verbose=0
    )   # (Nseg, 4)
