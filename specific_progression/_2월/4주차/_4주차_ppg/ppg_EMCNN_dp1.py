# Case 1. 각 채널을 개별적으로 처리하는 경우 (3채널 케이스)
import os
import glob
import numpy as np
...
def build_feature_extractor(input_length=None):
    """
    입력 shape: (L, 1) – L: 시퀀스 길이
    출력: 128 차원 feature 벡터 (GlobalAveragePooling1D 사용)
    """
    inputs = tf.keras.Input(shape=(input_length, 1))  # 각 샘플은 단일 채널
...
class EMCNN(tf.keras.Model):
    def __init__(self, num_classes=4, smoothing_kernel_size=2, downsample_factor=2, input_length=None):
        ...
        self.feature_downsampling = build_feature_extractor(input_length=None)  # downsampled branch

        # Smoothing: 고정 가중치 이동평균 필터 (입력 채널=1)
        self.smoothing_conv = tf.keras.layers.Conv1D(filters=1, kernel_size=smoothing_kernel_size,
                                                     padding='same', use_bias=False)
        # 필터 가중치 초기화 (모든 값 = 1/smoothing_kernel_size)
        smoothing_weight = np.ones((smoothing_kernel_size, 1, 1), dtype=np.float32) / smoothing_kernel_size
        self.smoothing_conv.build((None, input_length, 1))
        ...
        self.fc3 = tf.keras.layers.Dense(num_classes)  # 로짓 출력

    def call(self, inputs, training=False):
        """
        inputs: shape = (batch, L, 1)
        """
        # Transformation Stage:
        # 1. Identity Branch: 원본 그대로 사용
        branch_identity = inputs

        # 2. Smoothing Branch: 이동평균 필터 적용
        branch_smooth = self.smoothing_conv(inputs)

        # 3. Downsampling Branch: 시간축에서 downsample_factor 간격으로 샘플링
        branch_down = inputs[:, ::self.downsample_factor, :]

        # Feature Extraction Stage: 각 브랜치별 CNN 적용
...
def train_model_for_subject(subject_id, X, y, num_classes=4, epochs=50, batch_size=32,
                            ...
                            result_dir_base='EMCNN_IndividualChannel_Results'):
    """
    subject_id: 예, 's01'
    X: 입력 데이터, shape = (num_samples, 1280, 1) 해당 서브젝트 데이터
...
def load_data(data_dir):
    ...
    각 파일은 (51, 1, 1280) 모양이어야 하며, 각 파일에서 51개 전체 segment 대신  
    균일하게 6개 segment를 선택하여 (6, 1280, 1)로 변환한 후,  
    ...
    for file_path in file_paths:
        ...
        try:
            subject_id = base.split('_')[0]  # 예: 's01'
            label_str = base.split('_')[-1].split('.')[0]  # 예: '0'
            label = int(label_str)
        except Exception as e:
            print("라벨/서브젝트 추출 오류:", file_path, e)
            continue
        data = np.load(file_path)  # shape: (51, 1, 1280)
        # 각 segment: (1, 1280) -> (1280, 1)
        data = np.transpose(data, (0, 2, 1))  # (51, 1280, 1)
        # 각 파일에서 균일하게 6개 segment 선택 (예: 인덱스 0, 10, 20, 30, 40, 50)
        indices = np.linspace(0, data.shape[0]-1, 6, dtype=int)
        data = data[indices, :, :]  # shape: (6, 1280, 1)
...
if __name__ == "__main__":
    data_dir = '/home/bcml1/2025_EMOTION/DEAP_PPG_10s_1soverlap_4label/'  # .npy 파일들이 저장된 폴더 경로
    result_dir_base = 'EMCNN_IndividualChannel_Results'
    ...
    for subject_id in sorted(subject_data.keys(), key=lambda x: int(x[1:])):
        X = subject_data[subject_id]['X']  # shape: (total_samples, 1280, 1)
        y = subject_data[subject_id]['y']  # shape: (total_samples,)
        print(f"--- {subject_id} 학습 시작 ---")
        train_model_for_subject(subject_id, X, y, num_classes=num_classes, epochs=50, batch_size=32,
