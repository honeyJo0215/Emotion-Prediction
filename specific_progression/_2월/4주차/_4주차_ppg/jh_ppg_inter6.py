...
def preprocess_ppg_trial(trial_data, fs=128):
    ...
    (여기서는 PPG의 유의 주파수 대역 0.5 ~ 8 Hz 사용)
    """
    processed_segments = []
...
def subject_wise_normalize(X):
    """
    피험자별 전체 PPG 데이터(X)의 평균과 표준편차를 구해 정규화합니다.
    X shape: (num_samples, time_steps, num_channels)
    """
...
def load_ppg_data(data_dir='/home/bcml1/2025_EMOTION/DEAP_PPG_1s', 
    ...
    for file_path in file_paths:
        ...
        # (60, 5, 128)인 경우 transpose하여 (60, 128, 5)
        data = np.transpose(data, (0, 2, 1))
        data = data[:, :ppg_segment_length, :]  # 필요한 길이 만큼 슬라이싱
        
        # ===== 전처리: 각 trial의 모든 segment에 대해 처리 =====
        ...
        for start in range(0, num_segments - window_size + 1, step):
            ...
            trial_ids_list.append(trial_id)  # 해당 trial id 부여
    ...
    for subject in subject_data:
        ...
        # << Inter-subject 성능 향상을 위한 피험자별 정규화 적용 >>
...
# - 마지막 convolution 이후 SE block 적용
# =============================================================================
...
# =============================================================================
# PPG 특징 추출기: 각 채널별 branch 처리 후 fc layer 구성
# =============================================================================
...
# =============================================================================
# PPG 단일 모달 모델 생성:
# - 입력 직후 BatchNormalization을 적용해 raw 신호 분포 정규화
# =============================================================================
...
# =============================================================================
# 단일 모달 (PPG) 데이터 증강: 단일 modality에 대해
# (추후 시간 워핑, 진폭 스케일링 등 추가 증강 기법도 고려 가능)
# =============================================================================
...
def train_ppg_model_leave_one_subject_out(test_subject_id, train_ppg_X, train_y,
                                          ...
                                          result_dir_base="/home/bcml1/sigenv/_4주차_ppg/jh_result6_inter_seg10s"):
    result_dir = os.path.join(result_dir_base, f"test_{test_subject_id}")
    os.makedirs(result_dir, exist_ok=True)
    
    # training/validation split (stratify 기준 유지)
...
# =============================================================================
# Main 함수: PPG 데이터 로딩 후, leave-one-subject-out 방식으로 단일 모달 학습 및 평가
# =============================================================================
def main():
    ppg_data_dir = "/home/bcml1/2025_EMOTION/DEAP_PPG_1s"
    
    # PPG 데이터 로딩 (WINDOW_SIZE, SLIDING_STEP 동일 적용)
