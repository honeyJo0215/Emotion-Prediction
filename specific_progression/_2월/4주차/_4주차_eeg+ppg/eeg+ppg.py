...
# 결합은 각 1초 세그먼트를 채널 차원(axis=-1)로 이어붙임.
# =============================================================================
...
# =============================================================================
# subject별 멀티모달 모델 학습 (Stratified 5-Fold Cross Validation)
# =============================================================================
def train_multimodal_model_for_subject(subject_id, eeg_X, ppg_X, y, num_classes=4, epochs=150, batch_size=32,
                                       result_dir_base="/home/bcml1/sigenv/_4주차_eeg+ppg/result1"):
    ...
    for train_index, test_index in skf.split(eeg_X, np.argmax(y, axis=1)):
        ...
        # 모델 생성 및 컴파일
        eeg_input_shape = eeg_X.shape[1:]   # 예: (8, 9, 80) (10초로 결합된 채널)
...
def main():
    ...
    for subject in subjects:
        ...
        # 두 라벨은 동일하다고 가정 (여기서는 eeg_y 사용)
        y = eeg_y_onehot
        
        print(f"{subject} - Aligned samples: {min_samples}")
        
        train_multimodal_model_for_subject(subject, eeg_X, ppg_X, y, num_classes=4,
