...
def visualize_eeg_signal(file_path, save_dir, width_scale=10):
    ...
    파일명 형식: sXX_trial_XX_label_X.npy
      - 파일의 shape는 (60, 5, 128)입니다.
      - 60: segment 개수 → 각 채널별로 60개의 segment를 이어붙여 하나의 긴 파형으로 생성 (x축은 0~60초).
      - 5: 채널 수 → 각 채널에 해당하는 파형을 별도의 subplot으로 표시.
      - 최종적으로 5개의 subplot(채널 파형)이 아래로 순서대로 배치된 하나의 PNG 파일로 저장됨.
      
    저장 경로는 save_dir/DEAP_PPG/sXX/ 폴더 내에, 원본 파일명을 기반으로 예를 들어,
    "s01_trial_02_label_1.png"와 같이 저장됩니다.
    
    width_scale를 통해 figure의 가로 크기를 확대합니다. (기본값 10: 원래보다 10배 넓게)
    """
    # npy 파일 로드
    data = np.load(file_path)  # data.shape: (60, 5, 128)
    ...
    fig.suptitle(f'PPG Waveforms for {subject_id} (Trial {trial_num}, Label {label})', fontsize=16)
...
input_dir = "/home/bcml1/2025_EMOTION/DEAP_PPG_1s_nonorm"
# 파일 패턴: sXX_trial_XX_label_X.npy 형식의 파일들
file_pattern = os.path.join(input_dir, "s??_trial_*_label_*.npy")
file_list = glob.glob(file_pattern)

# 결과 저장 경로
save_dir = "/home/bcml1/sigenv/_4주차_data_check/DEAP_PPG_1s_nonorm_Visualize/"
