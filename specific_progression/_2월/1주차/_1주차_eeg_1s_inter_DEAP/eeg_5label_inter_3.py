...
SAVE_PATH = "/home/bcml1/sigenv/_1주차_eeg_1s_inter_DEAP/result3"
...
# 피실험자별 데이터 로드 함수
# 파일명 예: "sXX_sample_XX_segment_XXX_label_[Emotion]_2D.npy"
    ...
    for file_name in os.listdir(data_path):
        if file_name.startswith(subject) and file_name.endswith("_2D.npy"):
            ...
            if segment_name in ["000", "001", "002"]:
                # 필요에 따라 특정 segment 건너뛰기
                continue
            # 파일명에서 감정 라벨은 마지막에서 두 번째 항목
            emotion_label = parts[-2]
            if emotion_label in EMOTION_MAPPING:
                label = EMOTION_MAPPING[emotion_label]
                file_path = os.path.join(data_path, file_name)
                try:
                    de_features = np.load(file_path)  # 예: (4, 6, 6)
