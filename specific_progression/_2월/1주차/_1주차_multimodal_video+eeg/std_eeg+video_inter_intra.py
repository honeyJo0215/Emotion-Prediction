# inter subject multi-modal transformer(뇌파+eye crop video)(inter 모달리티 사용 후 intra모달리티)
# Inter → Intra는 모달리티 특성이 크게 다를 때 유용함
# Inter-Modality → Intra-Modality: 모달리티 간의 상호작용을 먼저 학습한 후, 각각의 모달리티에서 추가적인 Feature Refinement 적용

# EEG ↔ Eye Crop Video 간 Cross-Attention 적용 (Inter): 두 모달리티 간 관계를 먼저 학습하여, 공통적인 중요한 Feature를 추출
# EEG Feature Extraction (Intra): EEG 내에서 학습된 중요 Feature를 기반으로 추가 Refinement 진행
# Eye Crop Video Feature Extraction (Intra): Eye Video에서도 공통적으로 중요한 Feature를 강조
# Fusion 후 Classification
...
SAVE_PATH = "/home/bcml1/sigenv/_multimodal_video+eeg/result_stand_modelinter_interintra"
...
# EEG CNN 모델: CNN 기반 Feature Extraction + Dense Projection
class EEGCNN(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        x = self.conv3(x)
        x = self.flatten(x)
        return self.dense_proj(x)  # (batch, d_model)

# Eye Crop Video Feature Extractor (CNN 기반)
def build_eye_crop_model(d_model=64):
    eye_input = Input(shape=(50,8,64,3))
...
# ----------------------------------------------------
# 2. Transformer Encoder (Self-Attention) – 공통 모듈
# ----------------------------------------------------
...
def train_multimodal():
    subjects = [f"s{str(i).zfill(2)}" for i in range(8, 23)]
    for subject in subjects:
        ...
        #checkpoint_dir = f"/home/bcml1/sigenv/_multimodal_video+eeg/checkpoint/interintra_{subject}"
