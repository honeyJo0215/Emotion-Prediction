...
SAVE_PATH = "/home/bcml1/myenv/DEAP_Multimodal_cross2"
...
class CrossModalTransformer(tf.keras.layers.Layer):
    def __init__(self, input_dim, hidden_dim=128, num_heads=4):
        ...
        self.ffn = tf.keras.Sequential([
            ...
            Dense(input_dim)  # Output shape should match input_dim
        ])
    ...
    def call(self, query_input, key_value_input):
        # Cross-modal attention (e.g., EEG Query, Eye Key/Value)
        attention_output = self.cross_attention(query=query_input, key=key_value_input, value=key_value_input)
        attention_output = self.layer_norm1(query_input + attention_output)  # Residual connection
        
        # Feedforward network
        ffn_output = self.ffn(attention_output)
        return self.layer_norm2(attention_output + ffn_output)  # Residual connection
...
def build_multimodal_model(eeg_input_shape, eye_input_shape=(50, 8, 64, 3)):
    ...
    eeg_transformed = eeg_transformer(expand_dims_layer(eeg_features))  # (batch, 64, 64)
    eye_transformed = eye_transformer(expand_dims_layer(eye_features))  # (batch, 64, 64)

    # ✅ Cross-Modal Transformers
    cross_modal_eeg_q_eye_kv = CrossModalTransformer(input_dim=64)(eeg_transformed, eye_transformed)  # EEG Query, Eye KV
    cross_modal_eye_q_eeg_kv = CrossModalTransformer(input_dim=64)(eye_transformed, eeg_transformed)  # Eye Query, EEG KV

    # ✅ Concatenate Features (4가지 Transformer 결과)
    inter_input = Concatenate(axis=-1)([eeg_transformed, eye_transformed, cross_modal_eeg_q_eye_kv, cross_modal_eye_q_eeg_kv])  # (batch, 64, 256)
...
def train_multimodal():
    subjects = [f"s{str(i).zfill(2)}" for i in range(19, 23)]
