...
SAVE_PATH = "/home/bcml1/sigenv/_1주차_eeg_1s_intra_DEAP/test_result7_re"
...
#             f.write(test_report)
def train_subjects_with_kfold():
    subjects = [f"s{str(i).zfill(2)}" for i in range(30, 33)]   #이전에 01대신 11을 써서 01부터가 아닌 피실험자 11부터 시작하였음.(잘못 작성한 것)
    final_model_path = os.path.join(SAVE_PATH, "final_model")
    os.makedirs(final_model_path, exist_ok=True)
    ...
    for subject in subjects:
        ...
        for fold, (train_index, val_index) in enumerate(kf.split(train_data)):
            ...
            train_dataset = tf.data.Dataset.from_tensor_slices((train_data[train_index], train_labels[train_index])).batch(8).shuffle(1000)
            val_dataset = tf.data.Dataset.from_tensor_slices((train_data[val_index], train_labels[val_index])).batch(8)

            model = TransformerEncoder(input_dim=train_data.shape[-1], n_layers=2, n_heads=4, d_ff=512, p_drop=0.1, d_model=64)
            model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), loss="sparse_categorical_crossentropy", metrics=["accuracy"])

            model.fit(train_dataset, epochs=50, validation_data=val_dataset)

            # 폴드별 가중치 저장 (모델 전체가 아니라 가중치만 저장)
            fold_model_path = os.path.join(subject_save_path, f"{subject}_fold_{fold+1}.weights.h5")
            model.save_weights(fold_model_path)
            fold_model_paths.append(fold_model_path)

        # 평균 가중치 적용하여 최종 모델 생성
        final_model = TransformerEncoder(input_dim=train_data.shape[-1], n_layers=2, n_heads=4, d_ff=512, p_drop=0.1, d_model=64)
        final_model.build((None, train_data.shape[-1]))  # 모델을 먼저 빌드
        final_model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4), loss="sparse_categorical_crossentropy", metrics=["accuracy"])
        final_model = average_model_weights(final_model, fold_model_paths)
        ...
        test_report = classification_report(test_labels, predicted_labels, target_names=["Excited", "Relaxed", "Stressed", "Bored", "Neutral"], labels=[0, 1, 2, 3, 4], zero_division=0)
        print(f"\nTest Report for {subject}")
        print(test_report)
