...
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_eeg+pps_CNN/m_result1"
...
# =============================================================================
def self_attention_encoder(features, name_prefix):
    x = Lambda(lambda x: tf.expand_dims(x, axis=1))(features) 
    for i in range(2): 
        sa = MultiHeadAttention(num_heads=num_heads, key_dim=d_model, name=f"{name_prefix}SelfAttention{i+1}")( query=x, key=x, value=x) 
        sa = Dropout(dropout_rate)(sa) 
        x = Add()([x, sa]) 
        x = LayerNormalization(epsilon=1e-6)(x) 
    
    return Lambda(lambda x: tf.squeeze(x, axis=1))(x)
...
#             # 논문의 방식: 가중치를 적용한 Joint Loss Function
#             loss = LAMBDA1 * tf.reduce_mean(loss_inter) + \
#                    LAMBDA2 * tf.reduce_mean(loss_eeg) + \
#                    LAMBDA3 * tf.reduce_mean(loss_ppg)
...
#         return results
class MultimodalEmotionClassifier(tf.keras.Model):
    def __init__(self, base_model, lambda1, lambda2, lambda3, **kwargs):
        super(MultimodalEmotionClassifier, self).__init__(**kwargs)
        self.base_model = base_model
    ...
    def train_step(self, data):
        ...
        # x, y = data
        # y_true = y["Inter_Classification"]
        # y_eeg = y["EEG_Classification"]
        # y_ppg = y["PPG_Classification"]
        # y_inter = y["Weight_Softmax"]
        ...
        with tf.GradientTape() as tape:
            ...
            # Joint Loss 적용
            loss = self.lambda1 * tf.reduce_mean(loss_inter) + \
                   self.lambda2 * tf.reduce_mean(loss_eeg) + \
                   self.lambda3 * tf.reduce_mean(loss_ppg)
        ...
        return results
# ==========================
# 1️⃣ Grid Search 방식 수정 (반환값 활용)
# ==========================
def grid_search_lambda(train_data, val_data):
    ...
    for l1, l2, l3 in itertools.product(lambda_grid, repeat=3):
        ...
        model = MultimodalEmotionClassifier(build_combined_model(num_classes=3), l1, l2, l3)
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
                      loss="categorical_crossentropy",
                      metrics=["accuracy"])
        
        model.fit(*train_data, validation_data=val_data, epochs=3, batch_size=BATCH_SIZE, verbose=0)
        val_acc = model.evaluate(*val_data, verbose=0)[1]  
...
# ================================
# 2️⃣ Bayesian Optimization 방식 수정 (반환값 활용 + maxiter 추가)
# ================================
def bayesian_opt_lambda(train_data, val_data, max_iterations=15):
    def loss_function(lambdas):
        ...
        model.compile(
            ...
            loss={
                "Inter_Classification": "categorical_crossentropy",
                "EEG_Classification": "categorical_crossentropy",
                "PPG_Classification": "categorical_crossentropy",
                # "Weight_Softmax": lambda b_y_true, b_y_pred: tf.constant(0.0)
            },
            metrics=["accuracy"]
        )
    ...
    result = minimize(loss_function, x0=[0.5, 0.3, 0.2], 
...
def compute_class_weights(labels, num_classes=3):
    ...
    # 빈 클래스가 있는 경우 최소값을 1로 설정하여 NaN 방지
...
def train_multimodal(use_bayesian=False, max_iterations=20):
    ...
    for subject in subjects:
        ...
        if use_bayesian:
            best_lambdas = bayesian_opt_lambda(train_data, val_data, max_iterations=max_iterations)
        else:
            best_lambdas = grid_search_lambda(train_data, val_data)

        # 최적화된 가중치 적용
        lambda1, lambda2, lambda3 = best_lambdas
        ...
        model = MultimodalEmotionClassifier(base_model, lambda1, lambda2, lambda3)
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
                      loss={
        ...
        train_labels_dict = {
            ...
            "PPG_Classification": train_labels,
            "Weight_Softmax": np.zeros((train_labels.shape[0], 3))
        }
        valid_labels_dict = {
            "Inter_Classification": valid_labels,
            "EEG_Classification": valid_labels,
            "PPG_Classification": valid_labels,
            "Weight_Softmax": np.zeros((valid_labels.shape[0], 3))
        }
        ...
        history = model.fit(
            ...
            train_labels_dict,
            validation_data=(
                [valid_eeg, valid_ppg], valid_labels_dict
            ),
            epochs=EPOCHS,
