...
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_eeg+pps_CNN/result8"
...
# =============================================================================
# 커스텀 콜백: validation accuracy가 7 에포크 동안 개선되지 않으면 이전 최고 가중치 복원 후 학습 중단
# =============================================================================
...
# =============================================================================
# 9. 10-Fold Cross Validation을 적용한 학습 및 평가
# =============================================================================
def train_multimodal():
    ...
    for subject in subjects:
        ...
        for train_index, val_index in skf.split(train_eeg, train_labels):
            ...
            fold_train_ppg, fold_valid_ppg = train_ppg[train_index], train_ppg[val_index]
            fold_train_labels, fold_valid_labels = train_labels[train_index], train_labels[val_index]
            
            # 데이터 밸런싱 및 증강: 먼저 클래스별 균형 맞춤 후 전체 데이터 5배 증강
            fold_train_eeg, fold_train_ppg, fold_train_labels = balance_data(fold_train_eeg, fold_train_ppg, fold_train_labels)
            fold_train_eeg, fold_train_ppg, fold_train_labels = augment_all_data(
                fold_train_eeg, fold_train_ppg, fold_train_labels,
                noise_std_eeg=0.01, noise_std_ppg=0.005, factor=5)
            
            print(f"Fold {fold} 학습 샘플: EEG {fold_train_eeg.shape}, PPG {fold_train_ppg.shape}, 라벨 {fold_train_labels.shape}")
            ...
            history = model.fit(
                [fold_train_eeg, fold_train_ppg], fold_train_labels,
                validation_data=([fold_valid_eeg, fold_valid_ppg], fold_valid_labels),
                epochs=EPOCHS,
            ...
            scores = model.evaluate([fold_valid_eeg, fold_valid_ppg], fold_valid_labels, verbose=0)
        ...
        # 최종 test 평가 (원하는 경우 retraining 후 평가)
        base_model = build_combined_model(num_classes=3)
        model = MultimodalEmotionClassifier(base_model)
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
                      loss=focal_loss(alpha=compute_class_weights(train_labels)),
                      metrics=["accuracy"])
        train_eeg_aug, train_ppg_aug, train_labels_aug = balance_data(train_eeg, train_ppg, train_labels)
        train_eeg_aug, train_ppg_aug, train_labels_aug = augment_all_data(
            train_eeg_aug, train_ppg_aug, train_labels_aug,
            noise_std_eeg=0.01, noise_std_ppg=0.005, factor=5)
        model.fit([train_eeg_aug, train_ppg_aug], train_labels_aug,
