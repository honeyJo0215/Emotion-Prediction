...
# =============================================================================
# 커스텀 콜백: validation accuracy가 떨어지면 이전 최고 가중치 복원
# =============================================================================
class RestoreBestWeightsOnDrop(tf.keras.callbacks.Callback):
    def __init__(self, monitor='val_accuracy', min_delta=0.001, verbose=1):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current > self.best + self.min_delta:
            ...
            if self.best_weights is not None:
                if self.verbose:
                    print(f"\nValidation {self.monitor} dropped from {self.best:.4f} to {current:.4f}. Restoring best weights.")
...
def train_multimodal():
    ...
    for subject in subjects:
        ...
        # 먼저 원본 train 데이터를 클래스별로 균형 맞춤 (oversampling with replacement)
        train_eeg, train_ppg, train_labels = balance_data(train_eeg, train_ppg, train_labels)
        # 전체 데이터를 5배로 증강 (각 샘플마다 4개의 증강 샘플 생성)
        train_eeg, train_ppg, train_labels = augment_all_data(train_eeg, train_ppg, train_labels,
        ...
        # EarlyStopping와 RestoreBestWeightsOnDrop 콜백 적용
        callbacks = [
            RestoreBestWeightsOnDrop(monitor='val_accuracy', min_delta=0.001, verbose=1),
            tf.keras.callbacks.EarlyStopping(monitor='val_accuracy', patience=10, min_delta=0.001, restore_best_weights=True, verbose=1),
