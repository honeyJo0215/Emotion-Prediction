...
PPS_DATA_PATH = "/home/bcml1/2025_EMOTION/DEAP_PPS3" # PPS는 밴드(&로우)패스필터 적용 안한 input.
# PPS2 폴더의 파일은 필터 적용한 input. 두개다 써보고 결과를 비교해보자.
LABELS_PATH  = "/home/bcml1/2025_EMOTION/DEAP_four_labels"
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_new_EEG+PPS_CNN/m_4_2_result1"
...
def load_multimodal_data(subject):
    """
    각 피험자(subject)에 대해 EEG와 PPS 파일명을 기반으로 데이터를 로드합니다.
    파일명 형식: sXX_trial_XX_label_X.npy
      - EEG 파일 shape: (60, 32, 128)
      - PPS 파일 shape: (60, 8, 128)
    파일명에 있는 마지막 숫자(라벨)를 이용해 각 샘플의 라벨을 생성합니다.
    모든 파일의 1초 segment들을 연결한 후, train/test split을 진행합니다.
    """
    import glob
    # EEG 및 PPS 파일 패턴 (파일명 예: s01_trial_03_label_2.npy)
    ...
    for eeg_file in eeg_files:
        # 파일명 예: s01_trial_03_label_2.npy
        base = os.path.basename(eeg_file)
        parts = base.split('_')  # ["s01", "trial", "03", "label", "2.npy"]
        label_str = parts[-1]    # "2.npy"
        label = int(label_str.split('.')[0])  # 2
        eeg_data = np.load(eeg_file)  # shape: (60, 32, 128)
        eeg_list.append(eeg_data)
        # 각 파일의 60 샘플에 대해 동일 라벨을 부여
        labels_list.append(np.full((eeg_data.shape[0],), label, dtype=np.int32))
    
    # PPS 파일 로드 (파일 순서가 EEG와 동일하다고 가정)
    for pps_file in pps_files:
        pps_data = np.load(pps_file)  # shape: (60, 8, 128)
        pps_list.append(pps_data)
    
    # 파일별 데이터를 모두 이어붙임 (axis=0)
    eeg_array = np.concatenate(eeg_list, axis=0)      # shape: (num_files*60, 32, 128)
    pps_array = np.concatenate(pps_list, axis=0)        # shape: (num_files*60, 8, 128)
    labels_array = np.concatenate(labels_list, axis=0)  # shape: (num_files*60,)
    
    # 채널 차원 추가 (예: (num_samples, 32, 128, 1))
    eeg_array = np.expand_dims(eeg_array, axis=-1)
    pps_array = np.expand_dims(pps_array, axis=-1)
...
def train_multimodal(use_bayesian=False, max_iterations=20):
    ...
    for subject in subjects:
        ...
        for train_index, val_index in skf.split(train_eeg, train_labels):
            ...
            best_lambdas = (0.5, 0.3, 0.2)  # 고정된 lambda 값 사용(베이시안도 그리드서치도 사용하지 않기)
            ...
            callbacks = [
                tf.keras.callbacks.EarlyStopping(
                    ...
                    mode='max',  # 정확도는 최대화해야 하므로 'max'로 설정합니다.
                    patience=7,
        ...
        best_lambdas_final = (0.5, 0.3, 0.2)  # 고정된 lambda 값 사용(베이시안도 그리드서치도 사용하지 않기)
