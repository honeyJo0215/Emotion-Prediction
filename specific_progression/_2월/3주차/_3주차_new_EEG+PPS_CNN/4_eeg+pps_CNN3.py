...
# 4가지 감정 상태 라벨 파일 경로 (파일명: sXX_emotion_labels.npy)
LABELS_PATH  = "/home/bcml1/2025_EMOTION/DEAP_four_labels"
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_new_EEG+PPS_CNN/4_result3_3"
...
# =============================================================================
# 10. 최종 학습 및 평가 (논문 방식: 먼저 10-fold CV로 best weight 확보 → 해당 가중치로 150 에폭 최종 학습)
#     - 최종 학습에서는 early stopping 없이 150 에폭 모두 학습하며, 
#       validation split을 이용해 학습/validation curve를 기록합니다.
#     - test 데이터는 사용하지 않습니다.
# =============================================================================
def train_multimodal_final():
    ...
    for subject in subjects:
        ...
        # 10-fold cross validation을 수행하여 best fold의 가중치 확보
        ...
        for train_index, val_index in skf.split(eeg_array, labels):
            ...
            # fold 학습에서는 early stopping을 사용하여 빠르게 best weight 확보
            cv_callbacks = [
        ...
        # 최종 학습: 전체 데이터를 이용한 train/validation split (validation_split=0.2)
        # 초기 가중치는 CV에서 얻은 best_weights로 설정
        # 여기서는 early stopping 없이 150 에폭 모두 학습합니다.
        # (validation split은 학습 curve 기록 용으로만 사용)
        ...
        # 최종 학습 후 모델 가중치와 학습 curve만 저장 (test는 사용하지 않음)
        ...
        # 최종 요약 파일 저장 (학습 마지막 epoch의 train/val accuracy, loss 기록)
