...
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_new_EEG+PPS_CNN/4_final_2"
...
# =============================================================================
# 5. Loss 직접 지정용 함수 (focal loss) 및 lambda loss_weights 최적화를 위한 함수들
# =============================================================================
...
# def grid_search_lambda(train_data, val_data, alpha_values):
    ...
    #     if abs(l1 + l2 + l3 - 1.0) > 0.05:  # 합이 1에 가까운 경우만 사용
    #         continue

    #     model = build_combined_model(num_classes=4)
    ...
    #     eval_results = model.evaluate([valid_eeg, valid_pps], valid_labels_dict, verbose=2)
    #     # Inter_Classification_accuracy 위치를 확인
...
# =============================================================================
# 7. 데이터 증강 및 밸런싱 함수  
#    (※ factor를 기존 5배에서 2배로 변경하여, 각 샘플마다 1개의 증강 샘플 생성)
# =============================================================================
...
def balance_data(train_eeg, train_pps, train_labels):
    """
    각 클래스의 수를 동일하게 맞추기 위해 oversampling (with replacement) 수행.
    """
...
def augment_all_data(train_eeg, train_pps, train_labels, noise_std_eeg=0.01, noise_std_pps=0.005, factor=2):
    """
    각 샘플마다 (factor-1)개의 증강 샘플 생성 → 원본+증강 총 2배 데이터.
    """
    N = train_eeg.shape[0]
    aug_eeg, aug_pps, aug_labels = [], [], []
    for i in range(N):
        # factor=2 → 1개의 증강 샘플 생성
...
# =============================================================================
# 8. 데이터 로드 함수  
# 각 피험자별 파일들을 모두 이어붙여, (num_files*60, 32, 128, 1) 형태의 데이터를 생성
# =============================================================================
def load_multimodal_data(subject):
    ...
    각 파일의 60개의 샘플을 이어붙여 최종적으로 train/test split 진행.
    """
    ...
    for eeg_file in eeg_files:
        ...
        eeg_data = np.load(eeg_file)  # (60, 32, 128)
        eeg_list.append(eeg_data)
        labels_list.append(np.full((eeg_data.shape[0],), label, dtype=np.int32))
    
    # PPS 파일 로드 (EEG와 동일 순서로 있다고 가정)
    for pps_file in pps_files:
        pps_data = np.load(pps_file)  # (60, 8, 128)
        pps_list.append(pps_data)
    
    eeg_array = np.concatenate(eeg_list, axis=0)      # (num_files*60, 32, 128)
    pps_array = np.concatenate(pps_list, axis=0)        # (num_files*60, 8, 128)
    labels_array = np.concatenate(labels_list, axis=0)  # (num_files*60,)
    
    # 채널 차원 추가
    eeg_array = np.expand_dims(eeg_array, axis=-1)  # (N, 32, 128, 1)
    pps_array = np.expand_dims(pps_array, axis=-1)    # (N, 8, 128, 1)
    
    # train/test split (라벨별 균형 유지)
...
# =============================================================================
# 10. 최종 학습 및 평가 (10-fold Cross Validation + 최종 모델 학습)
# =============================================================================
def train_multimodal(use_bayesian=False, max_iterations=20):
    ...
    for subject in subjects:
        ...
        for train_index, val_index in skf.split(train_eeg, train_labels):
            ...
            # 폴드별 검증 평가
            eval_results = model.evaluate([fold_valid_eeg, fold_valid_pps], valid_labels_dict, verbose=2)
            # metric_index = model.metrics_names.index("Inter_Classification_accuracy")
            # fold_val_acc = eval_results[metric_index]
            # if "Inter_Classification_accuracy" in model.metrics_names:
            #     metric_name = "Inter_Classification_accuracy"
            # elif "Inter_Classification_sparse_categorical_accuracy" in model.metrics_names:
            #     metric_name = "Inter_Classification_sparse_categorical_accuracy"
            # else:
            #     raise ValueError("Inter Classification accuracy metric not found in model.metrics_names")
