...
# 결과 저장 폴더
output_folder = "/home/bcml1/2025_EMOTION/DEAP_EEG2"
...
for sub in subjects_list:
    ...
    eeg_segments = np.zeros((num_trials, num_channels, num_segments, WINDOW_SIZE))
    
    for segment_idx in range(num_segments):
        start = segment_idx * WINDOW_SIZE
        eeg_segments[:, :, segment_idx, :] = eeg_data_filtered[:, :, start:start + WINDOW_SIZE]
    ...
    mean_seg = np.mean(eeg_segments, axis=3, keepdims=True)
    std_seg = np.std(eeg_segments, axis=3, keepdims=True)
    eeg_segments_standardized = (eeg_segments - mean_seg) / (std_seg + 1e-8)
    
    # 8. 각 segment별로 저장 (파일 이름 형식: sXX_eeg_signals_segment_XX.npy)
    for segment_idx in range(num_segments):
        ...
        np.save(save_path, eeg_segments_standardized[:, :, segment_idx, :])
        print(f"   ✅ 저장 완료: {save_path} - Shape: {eeg_segments_standardized[:, :, segment_idx, :].shape}")
    
    # 메모리 정리
    del x, sub_data, eeg_data, eeg_data_cleaned, eeg_data_filtered, eeg_segments, eeg_segments_standardized
    gc.collect()
