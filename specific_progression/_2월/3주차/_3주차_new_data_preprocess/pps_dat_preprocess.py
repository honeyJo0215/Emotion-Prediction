...
# 결과 저장 폴더
output_folder = "/home/bcml1/2025_EMOTION/DEAP_PPS"
...
for sub in subjects_list:
    ...
    # pps_data_filtered = signal.filtfilt(b, a, pps_data_cleaned, axis=2)
    
    # 6. 1초 단위로 데이터 분할  
    #    전체 샘플 수: 7680 → 1초(128 샘플) 단위이면 총 7680/128 = 60 segment
    # num_trials, num_channels, num_samples = pps_data_filtered.shape
    num_trials, num_channels, num_samples = pps_data_cleaned.shape
    ...
    for segment_idx in range(num_segments):
        ...
        pps_segments[:, :, segment_idx, :] = pps_data_cleaned[:, :, start:start + WINDOW_SIZE]
    ...
    del x, sub_data, pps_data, pps_data_cleaned, pps_segments, pps_segments_standardized
    gc.collect()
