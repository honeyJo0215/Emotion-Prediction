...
def build_combined_model(num_classes=3):
    ...
    #     print("🔎 inter_pred.shape:", inter_pred.shape)
    #     print("🔎 eeg_pred.shape:", eeg_pred.shape)
    #     print("🔎 eye_pred.shape:", eye_pred.shape)
    ...
    # # ✅ 🔥 model.compile() 실행 여부 확인
    # print("✅ model.compile() 실행 완료")
    # for i, output in enumerate(model.outputs):
    #     print(f"🔎 Model Output {i}: {output.shape}")  # 모델 출력 shape 확인
...
class MultimodalEmotionClassifier(tf.keras.Model):
    ...
    def test_step(self, data):
        ...
        # 메트릭 업데이트 (Inter Classification branch 기준)
        self.compiled_metrics.update_state(y_true, inter_pred)
        metric_results = {m.name: m.result() for m in self.metrics}
        
        # 반환하는 딕셔너리의 키에 val_ 접두사 붙이기
        results = {("val_" + k): v for k, v in metric_results.items()}
...
def load_multimodal_data(subject):
    ...
    # ✅ Sample 1~5를 대상으로 데이터 로드 (여기서 반복 횟수가 sample의 개수)
...
#             # 전체 500프레임 구간 선택 (여기서는 frame_indices의 처음 500개)
#             selected_frames = frame_indices[:window_length]

#             # 만약 500프레임이 부족한 경우 마지막 프레임을 복제하여 500개로 맞춤
#             if len(selected_frames) < window_length:
#                 print(f"⚠ Warning: Found only {len(selected_frames)} frames in selected window for segment {segment_index:03d}")
#                 while len(selected_frames) < window_length:
#                     selected_frames.append(selected_frames[-1])
...
#                 if frame in file_mapping:
#                     eye_frame_files.append(file_mapping[frame])
#                 if len(eye_frame_files) == 100:  # ★ 100개로 수정
#                     break
...
#     # 실제 로드한 샘플 개수를 기준으로 분할 (여기서 stratify 인자를 추가)
...
#     train_eeg_data = [eeg_data[i] for i in train_indices]
#     train_eye_data = [eye_data[i] for i in train_indices]
#     train_labels  = [labels[i] for i in train_indices]
    
#     test_eeg_data = [eeg_data[i] for i in test_indices]
#     test_eye_data = [eye_data[i] for i in test_indices]
...
#     for sample_index in range(1, 41):  # 예제에서는 Sample 01만 사용
#         sample_number = f"{sample_index:02d}"
#         print(f"\n🟢 Processing {subject} - Sample {sample_number}")
...
#         # 기존 모델 생성 후 custom 모델 클래스에 래핑합니다.
#         base_model = build_combined_model(num_classes=3)
#         model = MultimodalEmotionClassifier(base_model)
#         model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
#                       metrics=["accuracy"])
...
#         # print("train_labels.shape:", train_labels.shape)  # (batch_size, 3) -> 정상
#         # print("valid_labels.shape:", valid_labels.shape)  # (batch_size, 3) -> 정상
...
#                     # # ✅ 학습 전에 한 번 예측 실행하여 손실 함수 디버깅
#                     # # ✅ 데이터 shape 확인
#                     # print("🔎 train_eeg[:2] shape:", train_eeg[:2].shape)
#                     # print("🔎 train_eye[:2] shape:", train_eye[:2].shape)

#                     # # ✅ 차원 확장 후 model.predict() 실행
#                     # test_output = model.predict([
#                     #     np.expand_dims(train_eeg[:2], axis=-1).astype(np.float32),  # ✅ 차원 확장 후 float32 변환
#                     #     train_eye[:2].astype(np.float32)
#                     # ])
