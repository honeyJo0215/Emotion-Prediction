...
# Stratify 적용, Focal Loss에서 사용하는 값 알파 감마를 하이퍼 파라미터 튜닝으로 구하기
...
import kerastuner as kt  # Keras Tuner 설치 필요 (pip install keras-tuner)
...
# 0. GPU 메모리 제한 (필요 시)
# =============================================================================
def limit_gpu_memory(memory_limit_mib=4000):
...
SAVE_PATH = "/home/bcml1/sigenv/_2주차_eye+eeg_CNN/result5_real"
...
# -------------------------
# 📌 모델 빌더 함수 (Keras Tuner 용)
def model_builder(hp, train_labels):
    """
    Keras Tuner를 활용한 하이퍼파라미터 튜닝 함수.
    외부에서 전달받은 train_labels를 이용해 alpha 값을 계산합니다.
    """
    # gamma 값 튜닝 (1.0 ~ 3.0)
    gamma = hp.Float('gamma', min_value=1.0, max_value=3.0, step=0.5, default=2.0)
    # alpha_scale 튜닝: 클래스 불균형을 반영한 alpha에 곱해줄 스케일 값
    alpha_scale = hp.Float('alpha_scale', min_value=0.5, max_value=2.0, step=0.1, default=1.0)
    
    # 외부에서 전달받은 train_labels로 alpha 자동 계산
    alpha_values = compute_class_weights(train_labels) * alpha_scale
    print(f"Tuning hyperparameters: gamma={gamma}, alpha={alpha_values}")
    
    base_model = build_combined_model(num_classes=3)
    model = MultimodalEmotionClassifier(base_model)
    
    # compile 시 focal loss를 사용합니다.
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
                  loss=focal_loss(alpha=alpha_values, gamma=gamma),
                  metrics=["accuracy"])
    return model
...
#         base_model = build_combined_model(num_classes=3)
#         model = MultimodalEmotionClassifier(base_model)
#         # focal loss를 사용하도록 custom train_step 내부에서 focal_loss_fn를 사용하기 위해
#         model.focal_loss_fn = focal_loss(alpha=alpha_values, gamma=2.0)
#         # Option 1: focal loss 사용 (커스텀 train_step에서 focal loss를 직접 적용)
...
#         # ✅ 클래스 빈도수 기반 alpha 계산 (기존 코드 그대로 사용)
#         # alpha_values = compute_class_weights(train_labels)

#         # --------------- 여기부터 하이퍼파라미터 튜닝 적용 ---------------
#         # Keras Tuner의 모델 빌더 함수를 이용하여 focal loss의 gamma와 각 클래스의 alpha 값을 튜닝합니다.
#         tuner = kt.RandomSearch(
#             hypermodel=partial(model_builder, train_labels=train_labels),                           # 미리 정의한 모델 빌더 함수
#             objective='val_accuracy',                # 검증 정확도를 기준으로 최적화
#             max_trials=2,                            # 시도할 하이퍼파라미터 조합 수 (필요에 따라 조정)
#             executions_per_trial=1,                  # 각 조합 당 실행 횟수
#             directory='kt_tuner_dir',                # 튜닝 결과를 저장할 디렉토리
#             project_name=f'{subject}_focal_loss_tuning'
#         )

#         print("Starting hyperparameter tuning ...")
#         tuner.search([train_eeg, train_eye], train_labels,
#                      validation_data=([valid_eeg, valid_eye], valid_labels),
#                      epochs=EPOCHS,
#                      batch_size=BATCH_SIZE)
#         tuner.results_summary()

#         # 최적의 하이퍼파라미터를 찾은 모델을 가져옵니다.
#         best_model = tuner.get_best_models(num_models=1)[0]
#         # -----------------------------------------------------------------

#         # 최적 모델의 성능 평가 (테스트 데이터)
#         test_loss, test_acc = best_model.evaluate([test_eeg, test_eye], test_labels, batch_size=BATCH_SIZE)
#         print(f"Test Loss: {test_loss}, Test Accuracy: {test_acc}")

#         # 테스트 데이터에 대해 예측 후 classification report 출력
#         predictions = best_model.predict([test_eeg, test_eye])
#         inter_pred = predictions[0]  # 평가에 사용할 분류 브랜치 선택
#         predicted_labels = np.argmax(inter_pred, axis=-1)
#         test_report = classification_report(
#             test_labels, predicted_labels,
...
# 학습 및 평가
def train_multimodal():
    subjects = [f"s{str(i).zfill(2)}" for i in range(15, 23)]
    for subject in subjects:
        print(f"Training subject: {subject}")
        train_eeg, train_eye, train_labels, test_eeg, test_eye, test_labels = load_multimodal_data(subject)
        ...
        optimal_gamma = best_hp.get('gamma')
        optimal_alpha_scale = best_hp.get('alpha_scale')
        optimal_alpha_values = compute_class_weights(train_labels) * optimal_alpha_scale
        print(f"Optimal gamma: {optimal_gamma}, Optimal alpha: {optimal_alpha_values}")
        
        # -----------------------------------------------------------------
        # 베스트 하이퍼파라미터를 반영하여 모델 생성 및 컴파일
        base_model = build_combined_model(num_classes=3)
        model = MultimodalEmotionClassifier(base_model)
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=LEARNING_RATE),
                      loss=focal_loss(alpha=optimal_alpha_values, gamma=optimal_gamma),
                      metrics=["accuracy"])
        model.summary()
