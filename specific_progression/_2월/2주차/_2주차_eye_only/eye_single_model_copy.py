...
SAVE_PATH = "/home/bcml1/sigenv/_2주차_eye_only/result_copy2"
...
def average_model_weights(model, model_paths):
    # 최종 모델도 dummy forward pass를 수행하여 빌드합니다.
    dummy_input = tf.zeros((1, model.window_length, 2, 32, 64, 3))
    _ = model(dummy_input)
    ...
    for path in model_paths:
        try:
            ...
            # 임시 모델도 dummy forward pass를 통해 빌드합니다.
            dummy_input_temp = tf.zeros((1, model.window_length, 2, 32, 64, 3))
            _ = temp_model(dummy_input_temp)
...
class EyeCropTransformer(tf.keras.Model):
    def __init__(self, window_length=50, n_layers=2, n_heads=4, d_ff=512, p_drop=0.1, d_model=128):
        """
        Args:
            window_length: 최종 입력 시퀀스 길이 (여기서는 50프레임)
            n_layers: Transformer encoder layer 수
            n_heads: Multi-head attention 헤드 수
            d_ff: Feed-forward 네트워크 차원
            p_drop: 드롭아웃 비율
            d_model: Transformer 임베딩 차원 (예: 128)
        """
        ...
        # Feature Extractor: 입력 프레임은 (2,32,64,3)을 채널 합쳐 (32,64,6)으로 변환 후 처리
    ...
    def call(self, inputs, training=False):
        ...
        x = inputs  # 이미 전처리된 데이터라 가정

        # --- 채널 합치기: (2,32,64,3) → (32,64,6) ---
        shape = tf.shape(x)
        batch_size = shape[0]
        time_steps = shape[1]
        # 배치와 시간 축 merge: (batch*time, 2,32,64,3)
        x_reshaped = tf.reshape(x, (-1, 2, 32, 64, 3))
        # (2,32,64,3) → (32,64,2,3)로 transpose
        x_transposed = tf.transpose(x_reshaped, perm=[0, 2, 3, 1, 4])
        # 마지막 두 차원을 합쳐 (32,64,6)으로 reshape
        x_combined = tf.reshape(x_transposed, (-1, 32, 64, 2*3))
        # 원래 배치와 시간 축으로 복원: (batch, time_steps, 32,64,6)
        x = tf.reshape(x_combined, (batch_size, time_steps, 32, 64, 6))

        # CNN 계층을 통한 feature 추출
        x = self.feature_extractor(x)       # → (batch, time_steps, 128)
        x = self.dense_projection(x)          # → (batch, time_steps, d_model)
...
def load_subject_eye_crop_data(subject, window_length=500, stride=50):
    ...
    - 각 subject 폴더 내에 40 trial의 npy 파일들이 있으며,
      각 trial는 여러 frame을 가진 npy 파일들로 구성됨.
    - 각 npy 파일은 (2,32,64,3)로 reshape됨.
    - 각 trial는 총 3000 프레임(예상)이며, 슬라이딩 윈도우 (window_length=500, stride=50)로 sample을 생성.
    - 단, 500프레임에서 매 10프레임마다 하나씩 골라서 50프레임을 균등하게 사용합니다.
    ...
    for trial in range(1, 41):
        ...
        for file_name in trial_files:
            ...
            try:
                ...
                continue
        trial_frames = np.array(trial_frames)  # (num_frames, 2,32,64,3)
        ...
        if num_frames < window_length:
            ...
            continue
        # 슬라이딩 윈도우 생성 후, 500프레임에서 매 10프레임마다 하나씩 골라 50프레임을 샘플링
        for start in range(0, num_frames - window_length + 1, stride):
            window = trial_frames[start:start + window_length]  # (500, 2,32,64,3)
            sampled_window = window[::10]  # 0,10,20,...,490 → (50, 2,32,64,3)
            all_windows.append(sampled_window)
    ...
    print(f"{subject}: Generated {data_array.shape[0]} samples from eye-crop data with shape {data_array.shape[1:]}.")
...
def train_eye_crop_with_stratified_kfold():
    ...
    for subject in subjects:
        ...
        try:
            # 여기서는 슬라이딩 윈도우 길이는 500이지만 최종적으로 50프레임으로 샘플링됨.
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(train_data, train_labels)):
            ...
            # 모델은 now 50프레임의 입력을 받도록 설정
            model = EyeCropTransformer(window_length=50, n_layers=2, n_heads=4, d_ff=512, p_drop=0.1, d_model=128)
            model.build((None, 50, 2, 32, 64, 3))
            model.compile(
            ...
            early_stopping = EarlyStopping(
                monitor='val_loss',  # 검증 데이터 손실을 기준으로 모니터링
                patience=5,          # val_loss가 5 epoch 동안 개선되지 않으면 종료
        ...
        # 가중치 평균화 후 최종 모델 생성
        final_model = EyeCropTransformer(window_length=50, n_layers=2, n_heads=4, d_ff=512, p_drop=0.1, d_model=128)
        final_model.build((None, 50, 2, 32, 64, 3))
        final_model.compile(
            optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4),
            loss="sparse_categorical_crossentropy",
            metrics=["accuracy"]
