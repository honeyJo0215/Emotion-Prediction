...
# =============================================================================
# 7. rPPG 데이터 로드 함수 (모든 13 ROI를 이어 붙여서, trial 단위로 51 segment 유지)
# -----------------------------------------------------------------------------
...
# =============================================================================
# 8. EEG & Eye 데이터 로드 함수 (500프레임 중 균일 샘플링으로 100프레임 선택)
# -----------------------------------------------------------------------------
...
def train_multimodal():
    for subject in SUBJECTS:
        ...
        # 주의: EEG/Eye 데이터의 sample 수는 세그먼트 단위이고, rPPG 데이터는 (num_trials*51,100,13,3)
        # 이들이 trial/segment 단위로 맞게 정렬되어 있다고 가정합니다.
        ...
        # rPPG 데이터도 train_test_split 적용 (stratify 없이 trial 순서대로 분할)
        rppg_train, rppg_test = train_test_split(rppg_data, test_size=0.2, random_state=42)
        # 여기서 rppg_train와 rppg_test는 각각 (num_train_segments,100,13,3)와 (num_test_segments,100,13,3)
        
        # Validation split for rPPG from train rppg
        rppg_train, rppg_valid = train_test_split(rppg_train, test_size=0.2, random_state=42)
        
        # **모든 모달의 train 샘플 수를 동일하게 맞추기**
