"""
DiffuSSM Noise + RandomForest Integrated Pipeline Example
- EEG CSP input → feature extraction with DiffuSSM → RandomForest classification
- Experiment: pre-train with src dataset → per-subject fine-tune and RF classification with tgt dataset
"""
import os, re, gc
import numpy as np
...
def load_deap_csp_features(csp_feature_dir="/home/bcml1/2025_EMOTION/DEAP_eeg_new_label_CSP"):
    """
    Loads all npy files in the specified directory.
    Filename format: folder{folder_num}_subject{subject}_sample{sample}_label{label}.npy
    Each file is of the form (4, T, 8), divided by 1 second (200 samples) to create (4,200,8) samples.
    Returns: X, Y, subjects, file_ids
    """
...
def load_seed_csp_features(csp_feature_dir="/home/bcml1/2025_EMOTION/SEED_IV/eeg_EM-CSP"):
    """
    Load all npy files in the specified directory.
    Filename format: folder{folder_num}_subject{subject}_sample{sample}_label{label}.npy
    Each file is of the form (4, T, 8), divided by 1 second (200 samples) to create (4,200,8) samples.
    Returns: X, Y, subjects, file_ids
    """
...
dirs = '/home/bcml1/2025_EMOTION/SEED_IV/eeg_EM-CSP'
datasets = [
