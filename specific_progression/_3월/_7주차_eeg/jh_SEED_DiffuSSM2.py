...
from tensorflow.keras.layers import (Input, Conv2D, MaxPooling2D, BatchNormalization, 
                                     ...
                                     Lambda, Flatten, TimeDistributed, Conv3D)
...
# -------------------------------------------------------------------------
# 기존 데이터 전처리 및 9x9 매핑 관련 helper 함수들
# -------------------------------------------------------------------------
...
def process_slice_9x9(slice_, channel_positions):
    # slice_: (62, 1, bands)
    slice_2d = np.squeeze(slice_, axis=1)  # (62, bands)
    mapped_9x9 = map_channels_to_2d_9x9(slice_2d, channel_positions)  # (9,9, bands)
    img = np.transpose(mapped_9x9, (2,0,1))  # (bands,9,9)
    img = np.expand_dims(img, axis=-1)        # (bands,9,9,1)
    return img
...
def process_trial_9x9(de_trial, psd_trial, channel_positions):
    ...
    for t in range(0, T, 2):
        ...
        for feature_idx in range(2):  # 0: DE, 1: PSD
            if feature_idx == 0:
                # 슬라이스 시 [t]를 사용하여 shape가 (62, 1, bands)가 되도록 함
                img1 = process_slice_9x9(slices_de[t], channel_positions)
                img2 = process_slice_9x9(slices_de[t+1], channel_positions)
            else:
                img1 = process_slice_9x9(slices_psd[t], channel_positions)
                img2 = process_slice_9x9(slices_psd[t+1], channel_positions)
            imgs1.append(img1)  # 각 img: (bands, 9, 9, 1)
            imgs2.append(img2)
        sample1 = np.stack(imgs1, axis=0)  # (2, bands, 9, 9, 1)
        sample2 = np.stack(imgs2, axis=0)  # (2, bands, 9, 9, 1)
        sample = np.expand_dims(np.stack([sample1, sample2], axis=0), axis=0)  # (1, 2, 2, bands, 9, 9, 1)
        sample_list.append(sample)
    return np.concatenate(sample_list, axis=0)

# -------------------------------------------------------------------------
# CNN+Transformer 관련 Layer들 (new code 유지)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# Diffusion 기반 Single Segment Model (입력: (2, 4, 9, 9, 1))
# -------------------------------------------------------------------------
def build_single_segment_model(d_model=64):
    # 2 모달리티: DE, PSD → 입력 shape: (2, 4, 9, 9, 1)
...
# -------------------------------------------------------------------------
# 최종 모델: TimeDistributed로 Single Segment 모델 적용 후 Transformer Encoder Layer 통과
# -------------------------------------------------------------------------
def build_model(input_shape=(2, 2, 4, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    ...
    모델 입력: (T, 2, 4, 9, 9, 1)
     - T = 2 타임스텝 (각각 4프레임씩)
     - 두 모달리티: DE와 PSD
     - 9x9 2D 매핑 이미지, 채널 수 1
    """
    inputs = tf.keras.layers.Input(shape=input_shape)
    single_seg_model = build_single_segment_model(d_model)
    features_seq = TimeDistributed(single_seg_model)(inputs)
    x = features_seq
...
# -------------------------------------------------------------------------
# GradualUnfreeze Callback, 채널 중요도 산출 및 시각화 함수 (변경 없음)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# LOSO 방식 inter-subject cross validation training (9x9 매핑 및 diffusion 모델 적용)
# -------------------------------------------------------------------------
def inter_subject_cv_training_9x9(base_dirs, result_dir,
    ...
    for test_subj in subjects:
        ...
        model.summary()
        pretrain_callbacks = [
            EarlyStopping(monitor='val_accuracy', patience=100, min_delta=0.0005, restore_best_weights=True)
        ]
...
RESULT_DIR = "/home/bcml1/sigenv/_7주차_eeg/jh_Diffu_2"
