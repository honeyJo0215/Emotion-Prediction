...
# -------------------------------------------------------------------------
# DiffuSSM 레이어: Hourglass, Bidirectional GRU (SSM 근사), AdaLN-Zero, FFN, Adapter 포함
# -------------------------------------------------------------------------
class DiffuSSMLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, d_ff, dropout_rate=0.5, bottleneck_dim=16, **kwargs):
        ...
        # Hourglass block: 다운샘플 후 업샘플 (Dense 레이어 이용)
        ...
        # Bidirectional GRU를 SSM 근사로 사용 (merge_mode='sum')
        ...
        # Adapter Module: 간단한 다운-업 레이어 (skip connection 포함)
        ...
        # AdaLN-Zero: 초기에는 SSM branch의 영향을 0으로
...
def process_slice_9x9(slice_, channel_positions):
    ...
    img = np.transpose(mapped_9x9, (2,0,1))  # (4,9,9) where 4 = #frequency bands
...
#함수들 정의
# DiffusionDenoiseLayer: 간단한 2D 평균 필터를 이용한 denoising layer
class DiffusionDenoiseLayer(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        ...
        self.conv = tf.keras.layers.Conv2D(
            ...
            trainable=False  # 고정된 필터
        )
    
    def call(self, inputs):
        # 입력이 4D 텐서라고 가정: (batch, height, width, channels)
        if inputs.shape[-1] == 1:
    ...
    def compute_output_shape(self, input_shape):
        # "same" padding이므로 공간 차원은 그대로 유지됩니다.
        return input_shape

# PreprocessLayer: 입력을 그대로 반환하는 간단한 전처리 layer (필요 시 수정 가능)
class PreprocessLayer(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super(PreprocessLayer, self).__init__(**kwargs)
    def call(self, inputs):
        # 예: 입력 정규화 (여기서는 간단히 identity 함수)
        return inputs

# Concatenate: tf.concat을 래핑한 custom layer (tf.keras.layers.Concatenate를 사용해도 무방)
...
class CNNBranchLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_bands=4, **kwargs):
        ...
        # 채널(주파수 밴드) 가중치: shape (num_bands, 1, 1, 1)
    ...
    def call(self, x):
        x = self.preprocess(x)
        # 적용: 각 주파수 밴드에 대해 학습 가능한 가중치를 곱함
...
# Gaussian 노이즈 추가 후 DiffusionDenoiseLayer를 적용하고, 이후 CNN branch로 특징 추출
# =============================================================================
def build_single_segment_model(d_model=64):
    ...
    # Diffusion Denoise: 기존 DiffusionDenoiseLayer 사용 (여기서 노이즈 제거 수행)
    de_denoised = TimeDistributed(DiffusionDenoiseLayer())(de_noisy)  # (4,9,9,1)
    psd_denoised = TimeDistributed(DiffusionDenoiseLayer())(psd_noisy)  # (4,9,9,1)
...
def build_model(input_shape=(2, 2, 4, 9, 9, 1), n_diff_layers=2, d_ff=512, p_drop=0.5, d_model=64, noise_std=0.1):
    """
    모델 입력: (T, 2, 4, 9, 9, 1)
      - T = 2: 4*2 = 8초 샘플(예: 8초 동안의 연속된 세그먼트)
      - 2: 2개의 모달리티 (DE, PSD)
      - 4: 각 세그먼트당 4 프레임 (4밴드)
