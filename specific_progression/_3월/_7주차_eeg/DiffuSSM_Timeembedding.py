...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_7주차_eeg/Diffu_Time_1"
...
# =============================================================================
# Reverse Diffusion Layer: 사용자 정의 레이어로 역확산 업데이트 구현
class ReverseDiffusionLayer(tf.keras.layers.Layer):
    def __init__(self, n_diffusion, d_model, d_ff, p_drop, beta_start=0.0001, beta_end=0.02, **kwargs):
        super(ReverseDiffusionLayer, self).__init__(**kwargs)
        ...
        self.betas = np.linspace(beta_start, beta_end, n_diffusion)
        self.alphas = 1 - self.betas
        self.alpha_bars = np.cumprod(self.alphas)
        self.time_embed_layer = TimeEmbedding(d_model)
        self.diffussm_layer = DiffuSSMLayer(d_model, d_ff, dropout_rate=p_drop)
        
    def call(self, x, training=False):
        T_diff = self.n_diffusion
        for t in range(T_diff, 0, -1):
            beta_t = tf.constant(self.betas[t-1], dtype=tf.float32)
            alpha_t = tf.constant(self.alphas[t-1], dtype=tf.float32)
            alpha_bar_t = tf.constant(self.alpha_bars[t-1], dtype=tf.float32)
            batch_size = tf.shape(x)[0]
            t_tensor = tf.fill([batch_size], tf.cast(t, tf.float32))
            t_emb = self.time_embed_layer(t_tensor)  # (batch, d_model)
            epsilon = self.diffussm_layer(x, t_emb, training=training)
            x = (1 / tf.sqrt(alpha_t)) * (x - ((1 - alpha_t) / tf.sqrt(1 - alpha_bar_t)) * epsilon)
        return x

# =============================================================================
# 최종 모델: TimeDistributed CNN으로 feature 추출 후, 
# diffusion forward로 노이즈 추가하고 PET 논문 기반의 역확산 과정을 수행하여 감정 분류
def build_model(input_shape=(10, 2, 4, 8, 8, 1), n_diffusion=10, d_ff=512, p_drop=0.5, d_model=64, noise_std=0.1):
    ...
    # 역확산 과정: ReverseDiffusionLayer 사용
    x = ReverseDiffusionLayer(n_diffusion, d_model, d_ff, p_drop)(noisy_features_seq)
