...
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_7주차_eeg/Diffu_diffusion_reverse1"
...
# 이 레이어는 기존 Transformer의 multi-head attention 대신 hourglass block, 
# gated bidirectional SSM (여기서는 Bidirectional GRU로 근사), 그리고 AdaLN-Zero 메커니즘을 적용하여,
# 노이즈가 추가된(즉, diffusion forward 과정으로부터 얻은) 시퀀스 feature를 역확산(denoising)하는 역할을 합니다.
class DiffuSSMLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, d_ff, dropout_rate=0.5, bottleneck_dim=16, **kwargs):
        ...
        # Gated bidirectional SSM을 Bidirectional GRU로 근사 (merge_mode='sum'으로 출력 차원 일치)
        ...
        # AdaLN-Zero: 초기에는 SSM branch 영향이 없도록 하기 위해 gating 파라미터를 0으로 초기화
    ...
    def call(self, inputs, training=False):
        ...
        # AdaLN-Zero 적용: 초기에는 alpha가 0이므로 SSM branch의 영향이 없도록 함
        ssm_out = self.alpha * ssm_out
        # Skip connection: 입력, hourglass branch, SSM branch 결합
...
# =============================================================================
# 최종 모델: TimeDistributed로 각 1초 segment를 처리한 후,
# diffusion 모델의 forward process를 모사하는 노이즈 추가 및
# DiffuSSM 레이어(역확산/denoising 역할)를 거쳐 최종 감정 분류 수행
def build_model(input_shape=(10, 2, 4, 8, 8, 1), n_layers=2, d_ff=512, p_drop=0.5, d_model=64, noise_std=0.1):
    ...
    # Diffusion forward: 노이즈 추가 (입력 데이터에 인위적 노이즈 주입)
    ...
    # 역확산(denoising) 및 조건부 복원: DiffuSSM 레이어를 n_layers 번 쌓음
