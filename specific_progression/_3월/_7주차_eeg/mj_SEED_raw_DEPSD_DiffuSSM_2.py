...
# ===== 데이터 증강: 기존에는 non-overlap 창 사용 → 이제 1초 stride (overlap) 사용 =====
...
# -------------------------------------------------------------------------
# 나머지 helper 함수들 (슬라이스 분리, 라벨 복제, 데이터 로드, 채널 매핑 등)
# -------------------------------------------------------------------------
...
class CNNBranchLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, num_bands=4, **kwargs):
        ...
        # self.preprocess = PreprocessLayer()  # 사용하지 않음
...
# -> 추가: 데이터 증강(슬라이딩 윈도우) 및 여러 Transformer layer 적용
# -------------------------------------------------------------------------
def build_model(input_shape=(10, 2, 4, 9, 9, 1), n_diff_layers=2, n_transformer_layers=1, d_ff=512, p_drop=0.5, d_model=64, noise_std=0.1):
    """
    모델 입력: (T, 2, 4, 9, 9, 1) with T=10 (10초 그룹)
      - 데이터 증강: 10초 창을 1초 오버랩으로 생성
      - Transformer layer: n_transformer_layers 개 반복 적용
    """
    ...
    # 이전 버전 (단일 Transformer layer 적용)
    # transformer_layer = TransformerEncoderLayer(d_model, n_heads=4, d_ff=d_ff, dropout_rate=p_drop)
    # x = transformer_layer(x)
    
    # 수정 버전: n_transformer_layers 개의 Transformer layer 반복 적용
...
def inter_subject_cv_training_9x9(base_dirs, result_dir, epochs=150, batch_size=16, target_subjects=None, pretrain_lr=3e-4, fine_tune_lr=3e-4, unfreeze_lr=1e-5):
    ...
    for test_subj in subjects:
        ...
        X_de_train_slices = [process_trial_9x9_new(de, psd, channel_positions_9x9, group_size=10) for de, psd in zip(X_de_train_split, X_psd_train_split)]
        y_train = replicate_labels_for_trials(X_de_train_slices, y_train_split)
        X_de_train = np.concatenate(X_de_train_slices, axis=0)
        X_de_val_slices = [process_trial_9x9_new(de, psd, channel_positions_9x9, group_size=10) for de, psd in zip(X_de_val_split, X_psd_val_split)]
        y_val = replicate_labels_for_trials(X_de_val_slices, y_val_split)
        X_de_val = np.concatenate(X_de_val_slices, axis=0)
        X_de_test_ft_slices = [process_trial_9x9_new(de, psd, channel_positions_9x9, group_size=10) for de, psd in zip(X_de_test_ft, X_psd_test_ft)]
        ...
        X_de_test_final_slices = [process_trial_9x9_new(de, psd, channel_positions_9x9, group_size=10) for de, psd in zip(X_de_test_final, X_psd_test_final)]
...
RESULT_DIR = "/home/bcml1/sigenv/_7주차_eeg/mj_rawDEPSD_DiffuSSM_2"
