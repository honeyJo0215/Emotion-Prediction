...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_7주차_eeg/Diffu_1"
...
# =============================================================================
# 새로 제안하는 DiffuSSM 레이어
# 기존 Transformer의 multi-head attention을 대체하여, hourglass layer와 
# gated bidirectional SSM(여기서는 Bidirectional GRU로 근사) 및 AdaLN-Zero 메커니즘을 적용합니다.
class DiffuSSMLayer(tf.keras.layers.Layer):
    def __init__(self, d_model, d_ff, dropout_rate=0.5, bottleneck_dim=16, **kwargs):
        ...
        # Hourglass block: 다운샘플 후 업샘플 (Dense 레이어 활용)
        ...
        # Gated bidirectional SSM을 Bidirectional GRU로 근사
        #self.bi_gru = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(d_model, return_sequences=True))
        ...
        # Adapter Module (기존 것 재사용)
        self.adapter = AdapterModule(d_model, bottleneck_dim)
        # AdaLN-Zero: SSM branch에 적용할 게이팅 파라미터, 초기값 0
    ...
    def call(self, inputs, training=False):
        ...
        # SSM branch (Bidirectional GRU로 근사)
        ssm_out = self.bi_gru(inputs, training=training)
        ssm_out = self.dropout(ssm_out, training=training)
        # AdaLN-Zero 적용: 초기에는 alpha가 0이므로 SSM branch 영향이 없음
        ssm_out = self.alpha * ssm_out
        # 입력, hourglass, SSM branch의 skip connection
...
# =============================================================================
# 최종 모델: TimeDistributed로 각 1초 segment를 처리한 후 DiffuSSM 레이어 적용
def build_model(input_shape=(10, 2, 4, 8, 8, 1), n_layers=2, d_ff=512, p_drop=0.5, d_model=64):
...
# =============================================================================
# Fine tuning 분리 함수 (파일별로, 중간 20% 구간 분리)
def split_finetune_test_by_file(X_test, Y_test, F_test, ft_ratio=0.2):
    ...
    for f in unique_files:
        ...
        n = len(indices)
        # 파일 내 윈도우 수가 너무 작으면 건너뛰기
        if n < 2:
            continue
        start = int(n * 0.4)
        end = int(n * 0.6)
        # 분할 결과가 비어 있으면 조정 (예: 마지막 윈도우 하나를 fine tuning으로)
        if end <= start:
            ft_idx = indices[-1:]
            test_idx = indices[:-1]
        else:
        ...
        # fine tuning과 test 데이터 모두 있는 경우에만 추가
    ...
    # 만약 아무런 파일에서도 fine tuning 데이터가 생성되지 않았다면 빈 배열 반환
    if len(X_ft_list) == 0:
...
# =============================================================================
# LOSO 방식: 선택한 subject 범위 내에서, 각 subject를 leave-one-out으로 test로 사용
