...
# -----------------------------------------------------------------------------
# 메인: epoch 후보를 모두 실행 (예: 100, 150, 200)
# -----------------------------------------------------------------------------
base_dirs = [
...
# epoch 후보 리스트 (예: 100, 150, 200)
epoch_candidates = [100, 150, 200, 300]

# 모든 epoch 후보에 대해 실행 (나머지 하이퍼파라미터는 고정)
for epochs in epoch_candidates:
    # 결과 저장 디렉터리 형식: epochs_[epochs]
    epoch_dir = f"epochs_{epochs}"
    RESULT_DIR = f"/home/bcml1/sigenv/_5주차_eeg_SEEDIV/DE+PSD_up2_inter_epoch_check/{epoch_dir}"
    print(f"\n=== Running experiment with epochs={epochs} ===")
    inter_subject_cv_training(base_dirs, RESULT_DIR, epochs=epochs, batch_size=16, target_subjects=TARGET_SUBJECTS,
                              pretrain_lr=3e-4, fine_tune_lr=3e-4, unfreeze_lr=3e-4, dropout_rate=0.3)