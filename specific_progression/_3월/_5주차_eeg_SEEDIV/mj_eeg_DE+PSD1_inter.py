...
# -----------------------------------------------------------------------------
# Inter-subject Leave-One-Subject-Out training
# -----------------------------------------------------------------------------
def inter_subject_cv_training(base_dirs, result_dir, epochs=150, batch_size=16):
    """
    SEED‑IV 데이터셋에서 전체 15개 subject 중 한 subject를 테스트셋(leave‑one‑subject‑out)으로
    사용하고, 나머지 subject의 trial을 합쳐 train/validation으로 분할하여 학습 및 평가합니다.
    각 테스트 subject별 결과는 sXX 폴더에 저장되며, 전체 결과는 overall 폴더에 저장됩니다.
    
    Parameters:
      base_dirs: list, 데이터가 저장된 경로 리스트.
    ...
    for test_subj in subjects:
        ...
        # 각 trial 데이터를 시간 슬라이스로 분리 (각 trial: (62, T, 4) → 시간 슬라이스: (62, 1, 4))
        ...
        # 각 trial마다 라벨 복제 (trial의 시간 길이에 따라 복제)
        ...
        subject_acc[test_subj] = test_acc
        ...
        subject_reports[test_subj] = report
    ...
    with open(overall_report_path, "w") as f:
        f.write("Overall Leave-One-Subject-Out Test Accuracy: {:.4f}\n\n".format(overall_acc))
        for subj in sorted(subject_reports.keys(), key=lambda x: int(x) if x.isdigit() else x):
...
RESULT_DIR = "/home/bcml1/sigenv/_5주차_eeg_CNN+TRAN/DE+PSD1_inter"
