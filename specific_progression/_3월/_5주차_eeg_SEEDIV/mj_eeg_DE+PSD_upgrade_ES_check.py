...
# -----------------------------------------------------------------------------
# CNN 모델 생성 함수 (pretrain_lr와 dropout_rate를 인자로 받음)
# -----------------------------------------------------------------------------
def create_seediv_cnn_model(input_shape=(62, 1, 4), num_classes=4, pretrain_lr=3e-4, dropout_rate=0.3):
    # DE branch
...
# (pretrain_lr, fine_tune_lr, unfreeze_lr, dropout_rate, 그리고 EarlyStopping 파라미터를 인자로 받음)
# -----------------------------------------------------------------------------
def inter_subject_cv_training(base_dirs, result_dir, epochs=150, batch_size=16, target_subjects=None,
                              pretrain_lr=3e-4, fine_tune_lr=3e-4, unfreeze_lr=3e-4, dropout_rate=0.3,
                              es_patience=20, es_min_delta=0.001):
    ...
    for test_subj in subjects:
        ...
        # 모델 생성 (pretrain_lr 및 dropout_rate 적용)
        model = create_seediv_cnn_model(input_shape=(62, 1, 4), num_classes=4, pretrain_lr=pretrain_lr, dropout_rate=dropout_rate)
        model.summary()

        # ----- 1. Pre-training Phase -----
        pretrain_epochs = int(epochs * 0.6)
        finetune_epochs = epochs - pretrain_epochs
        pretrain_callbacks = [EarlyStopping(monitor='val_accuracy', patience=es_patience, min_delta=es_min_delta, restore_best_weights=True)]
        ...
        # Gradual Unfreeze Callback (unfreeze_lr 적용)
...
# -----------------------------------------------------------------------------
# 메인: Early Stopping 후보를 모두 실행
# (예: es_patience: [10, 20, 30], es_min_delta: [0.001, 0.0005])
# -----------------------------------------------------------------------------
base_dirs = [
...
TARGET_SUBJECTS = [str(i) for i in range(1, 16)]  # Subject 1~15 대상

es_patience_candidates = [10, 20, 30]
es_min_delta_candidates = [0.001, 0.0005]

for es_patience in es_patience_candidates:
    for es_min_delta in es_min_delta_candidates:
        # 결과 저장 디렉터리 형식: ES_[patience]_[min_delta]
        es_dir = f"ES_{es_patience}_{es_min_delta}"
        RESULT_DIR = f"/home/bcml1/sigenv/_5주차_eeg_SEEDIV/DE+PSD_up2_inter_ES_check/{es_dir}"
        print(f"\n=== Running experiment with EarlyStopping: patience={es_patience}, min_delta={es_min_delta} ===")
...
# EarlyStopping 미적용 케이스
RESULT_DIR = "/home/bcml1/sigenv/_5주차_eeg_SEEDIV/DE+PSD_up2_inter_ES_check/No_ES"
print(f"\n=== Running experiment with No EarlyStopping ===")
inter_subject_cv_training(base_dirs, RESULT_DIR, epochs=150, batch_size=16, target_subjects=TARGET_SUBJECTS,
                          pretrain_lr=3e-4, fine_tune_lr=3e-4, unfreeze_lr=3e-4, dropout_rate=0.3,
                          es_patience=None, es_min_delta=None)