#8초의 DE feature + 8초의 PSD => 8초의 DE + PSD 가 입력으로 들어감.
...
def split_trial_to_time_slices(trial):
    ...
    trial.shape: (62, T, bands)
      - 62: 채널 수
      - T: time length (초 단위 또는 샘플 단위)
      - bands: 주파수 밴드 수 (예: 4 or 5)
    """
    channels, T, bands = trial.shape
    slices = [trial[:, t:t+1, :] for t in range(T)]  # 각 time step별로 자름
...
#     예: trial의 길이 T=4이면, 동일 라벨을 4번 복제
#     """
...
def replicate_labels_for_trials(trials_list, labels):
    """
    각 trial의 4초 슬라이스 T개에서 8초 샘플은 floor(T/2)개가 생성되므로,
    라벨도 해당 개수만큼 복제.
    """
...
# -------------------------------------------------------------------------
# [변경] 각 trial DE/PSD 데이터를 슬라이스별로 처리한 후,
# 2개의 연속 4초 슬라이스를 그룹화하여 8초 입력 (time dimension=2)으로 반환
# 반환 shape: (N, 2, 2, 4, 9, 9, 1) → N: 8초 샘플 수, 2: 시간 타임스텝, 2: [DE, PSD]
# -------------------------------------------------------------------------
def process_trial_9x9(de_trial, psd_trial, channel_positions):
    ...
    T = slices_de.shape[0]
    # 짝수 개의 슬라이스가 아니라면 마지막 한 개는 버림
    if T % 2 != 0:
        T = T - 1
    sample_list = []
    for t in range(0, T, 2):
        # 각 4초 슬라이스에 대해 DE와 PSD를 채널 축에서 결합 (62,2,4)
        ...
        for feature_idx in range(2):  # 0: DE, 1: PSD
            ...
            imgs1.append(img1)  # (4,9,9,1)
            imgs2.append(img2)
        sample1 = np.stack(imgs1, axis=0)  # (2,4,9,9,1)
        sample2 = np.stack(imgs2, axis=0)  # (2,4,9,9,1)
        # 두 4초 슬라이스를 시간축으로 묶어 8초 샘플 생성.
        # np.stack([sample1, sample2], axis=0) 의 결과는 (2,2,4,9,9,1)이 되는데,
        # 이 상태로 np.concatenate하면 첫 번째 차원이 샘플 수로 취급되어 2배가 됩니다.
        # 따라서, 한 쌍의 슬라이스마다 추가 차원을 부여하여 (1,2,2,4,9,9,1) 모양이 되도록 합니다.
        sample = np.expand_dims(np.stack([sample1, sample2], axis=0), axis=0)  # (1,2,2,4,9,9,1)
        sample_list.append(sample)
    # 각 trial의 결과를 concatenate하면 최종 shape는 (num_pairs, 2,2,4,9,9,1)
    # (즉, 한 trial에서 생성된 8초 샘플의 개수는 trial.shape[1]//2)
...
def build_single_segment_model(d_model=64):
    # 각 타임스텝 당 입력 shape: (2,4,9,9,1) → 2: [DE, PSD]
...
def build_model(input_shape=(2, 2, 4, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    ...
    여기서 T=2 (8초: 2 타임스텝, 각 4초)
    """
    inputs = tf.keras.layers.Input(shape=input_shape)
    single_seg_model = build_single_segment_model(d_model)
    # TimeDistributed를 통해 각 타임스텝마다 feature 추출 → (batch, T, d_model)
...
def inter_subject_cv_training_9x9(base_dirs, result_dir,
    ...
    for test_subj in subjects:
        ...
        # ---------------------------
        # 변경된 입력 shape: (2,2,4,9,9,1) → 2 타임스텝 (8초)
...
RESULT_DIR = "/home/bcml1/sigenv/_6주차_eeg_test_ft/mj_SEED_2Dmap_5"
