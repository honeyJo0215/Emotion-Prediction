...
def split_trial_to_time_slices_with_overlap(trial, window_size=4, overlap=0.5):
    """
    trial.shape: (62, T, bands)
      - 62: 채널 수
      - T: 총 시간 길이
    ...
    window_size: 슬라이딩 윈도우 크기 (기본 4초)
    overlap: 겹치는 비율 (50% -> 0.5)

    return:
      - shape: (new_T, 62, window_size, bands)
    """
    channels, T, bands = trial.shape
    stride = int(window_size * (1 - overlap))  # 50% overlap -> stride = window_size // 2

    slices = []
    for start in range(0, T - window_size + 1, stride):
        slices.append(trial[:, start:start+window_size, :])  # 윈도우 크기만큼 슬라이싱

    return np.array(slices)  # shape: (new_T, 62, window_size, bands)
...
def replicate_labels_for_trials(trials_list, labels):
    """
    trial.shape[1] (new_T)에 맞춰 label을 반복
    """
    replicated = [np.repeat(label[np.newaxis, :], trial.shape[0], axis=0) 
                  for trial, label in zip(trials_list, labels)]
    return np.concatenate(replicated, axis=0) if replicated else None
...
def process_trial_9x9(de_trial, psd_trial, channel_positions):
    # 50% overlap 적용하여 time slices 생성
    slices_de = split_trial_to_time_slices_with_overlap(de_trial, window_size=4, overlap=0.5)  # (new_T, 62, 4, bands)
    slices_psd = split_trial_to_time_slices_with_overlap(psd_trial, window_size=4, overlap=0.5)  # (new_T, 62, 4, bands)

    sample_list = []
    new_T = slices_de.shape[0]  # 변경된 time slice 개수
    ...
    return np.concatenate(sample_list, axis=0)  # (new_T, 2, 4, 9, 9, 1)
