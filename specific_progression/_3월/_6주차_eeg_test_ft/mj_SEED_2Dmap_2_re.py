...
# -------------------------------------------------------------------------
# -------------------------------------------------------------------------
# 수정된 process_trial_9x9 함수: DE와 PSD를 옆으로 이어붙여 (8,9,9,1) 모양으로 생성
# -------------------------------------------------------------------------
def process_trial_9x9(de_trial, psd_trial, channel_positions):
    # 슬라이스별 분리: (T, 62, 1, bands)
    slices_de = split_trial_to_time_slices(de_trial)    # shape: (T, 62, 1, 4)
    slices_psd = split_trial_to_time_slices(psd_trial)
    sample_list = []
    T = slices_de.shape[0]
    for t in range(T):
        # 각 슬라이스마다 2D 매핑 수행: 각각 (4,9,9,1)
        d_img = process_slice_9x9(slices_de[t], channel_positions)  # DE 이미지
        p_img = process_slice_9x9(slices_psd[t], channel_positions)  # PSD 이미지
        # 옆으로 이어붙여서 (8,9,9,1) 생성
        sample = np.concatenate([d_img, p_img], axis=0)
        # 시간 윈도우 차원 추가 → (1,8,9,9,1)
        sample = np.expand_dims(sample, axis=0)
        sample_list.append(sample)
    return np.concatenate(sample_list, axis=0)  # 최종 shape: (T,8,9,9,1)
...
# === 수정된 단일 segment 모델: 이제 입력 shape은 (8,9,9,1)
def build_single_segment_model(d_model=64):
    input_seg = tf.keras.layers.Input(shape=(8, 9, 9, 1))  # 8개의 밴드가 있는 이미지
    cnn_branch = CNNBranchLayer(d_model)
    features = cnn_branch(input_seg)  # (d_model,)
    # 필요한 경우 추가 Dense layer
    features = tf.keras.layers.Dense(d_model, activation="relu", kernel_regularizer=l2(1e-4))(features)
    model = tf.keras.models.Model(inputs=input_seg, outputs=features)
    return model

# === 수정된 최종 모델: 입력 shape을 (1,8,9,9,1)로 사용 (시간 윈도우=1)
def build_model(input_shape=(1, 8, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    """
    input_shape: (T=1, 8, 9, 9, 1)
    """
...
def compute_channel_importance(model, X_test_final, y_test, baseline_acc, 
    ...
    for ch in range(num_channels):
        ...
        if ch in channel_positions_9x9:
            ...
            # 새로운 shape에서는 spatial dimension이 index 3와 4에 위치
...
def inter_subject_cv_training_9x9(base_dirs, result_dir,
    ...
    for test_subj in subjects:
        ...
        # ---------------------------
        model = build_model(input_shape=(1, 8, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64)
        ...
        importance_map, channel_imp = compute_channel_importance(
            model=model,
            X_test_final=X_de_test_final,   # 올바른 입력 데이터 형태 (9x9 매핑 완료된 데이터)
            y_test=y_test_int,
