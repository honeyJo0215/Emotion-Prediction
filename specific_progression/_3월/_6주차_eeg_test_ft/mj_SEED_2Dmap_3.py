...
def replicate_labels_for_trials(trials_list, labels):
    """
    trials_list: EEG 데이터 리스트 (각 trial은 shape (62, T, 4))
    labels: 해당 trial의 라벨 (one-hot encoded)
    
    각 trial에 대해, process_trial_9x9 함수가 생성한 샘플 개수(T)만큼 라벨을 복제합니다.
    """
    replicated = [np.repeat(label[np.newaxis, :], trial.shape[1], axis=0)
                  for trial, label in zip(trials_list, labels)]
    return np.concatenate(replicated, axis=0) if replicated else None
...
def process_trial_9x9(de_trial, psd_trial, channel_positions):
    """
    각 trial에서 50% 오버랩으로 윈도우를 추출하여,
    각 윈도우에 대해 DE 슬라이스(4초)와 PSD 슬라이스(4초)를 처리한 후
    시간 축으로 이어 붙여 8초 길이의 입력 (8,9,9,1)을 만듭니다.
    
    Parameters:
      - de_trial: DE 데이터, shape=(62, T, bands)
      - psd_trial: PSD 데이터, shape=(62, T, bands)
      - channel_positions: 채널 위치 매핑 딕셔너리
      
    Returns:
      - X_output: 추출된 윈도우들의 배열, shape=(num_windows, 8, 9, 9, 1)
    """
    # trial을 시간 슬라이스들로 분리 (각 슬라이스 shape: (62, 1, bands))
    slices_de = split_trial_to_time_slices(de_trial)
    slices_psd = split_trial_to_time_slices(psd_trial)
    sample_list = []
    T = slices_de.shape[0]
    # 50% 오버랩: 한 슬라이스씩 이동 (step=1)
    step = 1  
    for t in range(0, T, step):
        # 여기서 각 t에 대해 DE와 PSD 슬라이스를 개별 처리하여
        # DE: (4,9,9,1), PSD: (4,9,9,1) 를 얻고,
        # 이들을 시간 축(axis=0)으로 이어 붙이면 (8,9,9,1)이 됨.
        img_de = process_slice_9x9(slices_de[t], channel_positions)  # (4,9,9,1)
        img_psd = process_slice_9x9(slices_psd[t], channel_positions)  # (4,9,9,1)
        img_combined = np.concatenate([img_de, img_psd], axis=0)       # (8,9,9,1)
        sample = np.expand_dims(img_combined, axis=0)                    # (1,8,9,9,1)
        sample_list.append(sample)
    X_output = np.concatenate(sample_list, axis=0)  # (num_windows, 8,9,9,1)
    print(f"✅ 생성된 EEG 데이터 개수: {X_output.shape[0]} (윈도우 개수)")
    return X_output

# def process_trial_9x9(de_trial, psd_trial, channel_positions, window_size=2, overlap=0.5):
#     """
#     window_size=2 (DE 4초 + PSD 4초 = 총 8초)
#     overlap 50% (한칸씩 이동)
#     """
...
#     step_size = 1  # 50% overlap → window가 2일 때 1씩 이동
#     T = slices_de.shape[0]

#     for t in range(0, T - 1, step_size):  # 한 칸씩 이동
#         combined_slice1 = np.concatenate([slices_de[t], slices_psd[t]], axis=1)  # (62, 2, 4)
#         combined_slice2 = np.concatenate([slices_de[t+1], slices_psd[t+1]], axis=1)  # (62, 2, 4)
...
#             img = np.concatenate([img1, img2], axis=0)  # (8, 9, 9, 1)
#             imgs.append(img)

#         sample = np.stack(imgs, axis=0)  # (2, 8, 9, 9, 1)
#         sample = np.expand_dims(sample, axis=0)  # (1, 2, 8, 9, 9, 1)
#         sample_list.append(sample)

#     return np.concatenate(sample_list, axis=0)  # (T-1, 2, 8, 9, 9, 1)
    # """
    # de_trial, psd_trial shape: (62, T, bands)
    # window_size=2 (DE 4초 + PSD 4초 = 총 8초)
    # overlap 50% (한칸씩 이동)
    # """
    # slices_de = split_trial_to_time_slices(de_trial)    # (T, 62, 1, bands)
    ...
    # step_size = 1  # 50% overlap → window가 2일때 1씩 이동
    # T = slices_de.shape[0]

    # for t in range(0, T - 1, 1):  # 한칸씩 이동 (50% overlap)
    #     combined_slice1 = np.concatenate([slices_de[t], slices_psd[t]], axis=1)      # (62, 2, 4)
    #     combined_slice2 = np.concatenate([slices_de[t+1], slices_psd[t+1]], axis=1)  # (62, 2, 4)
        
    #     imgs = []
    #     for feature_idx in range(2):  # DE(0), PSD(1)
    #         # 두 슬라이스를 합쳐 8초 윈도우 구성
    #         img1 = process_slice_9x9(combined_slice1[:, feature_idx:feature_idx+1, :], channel_positions)
    #         img2 = process_slice_9x9(np.concatenate([slices_de[t+1], slices_psd[t+1]], axis=1)[:, feature_idx:feature_idx+1, :], channel_positions)
            
    #         # 두 개를 시간 차원으로 합쳐서 8초로 구성
    #         img = np.concatenate([img1, img2], axis=0)  # (8, 9, 9, 1)
    #         imgs.append(img)
...
def build_model(input_shape=(1, 2, 8, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    """
...
def compute_channel_importance(model, X_test_final, y_test, baseline_acc, 
    ...
    for ch in range(num_channels):
        ...
        # test_dataset_mod = tf.data.Dataset.from_tensor_slices((X_test_mod, y_test)).batch(batch_size)
...
RESULT_DIR = "/home/bcml1/sigenv/_6주차_eeg_test_ft/mj_SEED_2Dmap_3"
