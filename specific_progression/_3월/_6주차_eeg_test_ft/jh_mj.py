...
def process_trial_9x9(de_trial, psd_trial, channel_positions, fixed_T=2):
    # 슬라이스 분리: (T, 62, 1, bands)
    slices_de = split_trial_to_time_slices(de_trial)
    slices_psd = split_trial_to_time_slices(psd_trial)
    sample_list = []
    
    # fixed_T 만큼의 슬라이스만 선택 (부족하면 패딩)
    for t in range(fixed_T):
        if t < slices_de.shape[0]:
            ...
            # t번째 슬라이스가 없는 경우, 동일한 shape의 0으로 채워진 배열 사용
            dummy_slice = np.zeros((de_trial.shape[0], 1, de_trial.shape[2]), dtype=np.float32)
            d_img = process_slice_9x9(dummy_slice, channel_positions)
            p_img = process_slice_9x9(dummy_slice, channel_positions)
        sample = np.stack([d_img, p_img], axis=0)  # (2,4,9,9,1)
        sample = np.expand_dims(sample, axis=0)      # (1,2,4,9,9,1)
        sample_list.append(sample)
    # 최종 shape: (fixed_T, 2,4,9,9,1)
    return np.concatenate(sample_list, axis=0)


# def process_trial_9x9(de_trial, psd_trial, channel_positions):
#     # 슬라이스별 분리: (T, 62, 1, 4)
#     slices_de = split_trial_to_time_slices(de_trial)    # shape: (T, 62, 1, bands)
#     slices_psd = split_trial_to_time_slices(psd_trial)
#     sample_list = []
#     T = slices_de.shape[0]
#     for t in range(T):
#         # 각 슬라이스마다 2D 매핑 수행 → (2, 4, 9, 9, 1)
#         d_img = process_slice_9x9(slices_de[t], channel_positions) #(4,9,9,1)
#         p_img = process_slice_9x9(slices_psd[t], channel_positions) #(4,9,9,1)
#         sample = np.stack([d_img, p_img], axis=0)  # (2, 4, 9, 9, 1)
#         # 여기서 추가 차원(시간 윈도우 길이=1) 추가: (1, 2, 4, 9, 9, 1)
#         sample = np.expand_dims(sample, axis=0)
#         sample_list.append(sample)
#     # 최종적으로 모든 슬라이스 샘플을 연결하여 반환
#     return np.concatenate(sample_list, axis=0)  # shape: (T, 2, 4, 9, 9, 1)
...
def build_model(input_shape=(None, 2, 4, 9, 9, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    """
...
def compute_channel_importance(model, 
                               X_de_test, X_psd_test, y_test, 
                               baseline_acc, 
                               channel_positions_9x9, 
                               batch_size=16):
    """
    - model: 이미 fine-tuning까지 끝난 학습된 모델
    - X_de_test, X_psd_test: shape (N, 62, 1, 4) 형태 (LOSO 최종 테스트 세트)
    - y_test: shape (N,) (또는 one-hot 이전의 라벨)
    - baseline_acc: 마스킹 없이 model.evaluate(...)로 얻은 기본 Test Accuracy
    - channel_positions_9x9: {채널인덱스: (row, col)} 형태의 딕셔너리
    - batch_size: 테스트 배치 크기
    """
    num_channels = 62
    ...
    for ch in range(num_channels):
        # DE 데이터를 복사해서 특정 채널만 0으로 세팅
        X_de_mod = np.copy(X_de_test)
        X_de_mod[:, ch, :, :] = 0.0  # 해당 채널의 모든 값 0으로
        
        # TensorFlow Dataset 구성
        test_dataset_mod = tf.data.Dataset.from_tensor_slices(((X_de_mod, X_psd_test), y_test)).batch(batch_size)
        
        # 모델 평가
        _, masked_acc = model.evaluate(test_dataset_mod, verbose=0)
        
        # 중요도: (기본 Acc - 마스킹 후 Acc)
...
def inter_subject_cv_training_9x9(base_dirs, result_dir,
    ...
    for test_subj in subjects:
        ...
        y_train_split = y_cat_train[train_idx]      # trial 단위 레이블 (예: (N, 4))
        y_val_split = y_cat_train[val_idx]            # trial 단위 레이블

        # 4) 각 trial별로 process_trial_9x9 적용 (fixed_T=2)
        # 최종 입력 shape: (num_trials, fixed_T, 2, 4, 9, 9, 1)
        X_de_train_slices = [
            process_trial_9x9(de, psd, channel_positions_9x9, fixed_T=2)
            for de, psd in zip(X_de_train_split, X_psd_train_split)
        ]
        X_de_train = np.array(X_de_train_slices)  # shape: (num_trials, 2, 2, 4, 9, 9, 1)
        y_train = y_train_split  # trial 단위 레이블 그대로 사용

        X_de_val_slices = [
            process_trial_9x9(de, psd, channel_positions_9x9, fixed_T=2)
            for de, psd in zip(X_de_val_split, X_psd_val_split)
        ]
        X_de_val = np.array(X_de_val_slices)  # shape: (num_trials, 2, 2, 4, 9, 9, 1)
        y_val = y_val_split

        # Fine-tuning 및 Final 테스트 데이터도 trial 단위로 처리
        X_de_test_ft_slices = [
            process_trial_9x9(de, psd, channel_positions_9x9, fixed_T=2)
            for de, psd in zip(X_de_test_ft, X_psd_test_ft)
        ]
        X_de_test_ft = np.array(X_de_test_ft_slices)
        # y_test_ft는 이미 trial 단위로 생성됨

        X_de_test_final_slices = [
            process_trial_9x9(de, psd, channel_positions_9x9, fixed_T=2)
            for de, psd in zip(X_de_test_final, X_psd_test_final)
        ]
        X_de_test_final = np.array(X_de_test_final_slices)
        # y_test_final 역시 trial 단위로 생성됨
        ...
        importance_map, channel_imp = compute_channel_importance(
            model=model,
            X_de_test=X_de_test_final,      # shape (N, 62, 1, 4)
            X_psd_test=X_psd_test_final,    # shape (N, 62, 1, 4)
            y_test=y_test_int,              # shape (N,)
