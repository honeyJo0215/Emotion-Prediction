...
# -----------------------------------------------------------------------------
# CNN 모델 생성 함수 (pretrain_lr를 인자로 받음)
# -----------------------------------------------------------------------------
...
# (pretrain_lr, fine_tune_lr, unfreeze_lr를 인자로 받음)
# -----------------------------------------------------------------------------
def inter_subject_cv_training(base_dirs, result_dir, epochs=150, batch_size=16, target_subjects=None,
    ...
    for test_subj in subjects:
        ...
        # 모델 생성 (pretrain_lr 적용)
        model = create_seediv_cnn_model(input_shape=(62, 1, 4), num_classes=4, pretrain_lr=pretrain_lr)
        model.summary()
...
# -----------------------------------------------------------------------------
# 메인: 네 가지 learning rate 후보를 모두 실행 (총 4^3 = 64가지 조합)
# -----------------------------------------------------------------------------
base_dirs = [
...
# 각 learning rate 후보 (1e-4 포함)
lr_candidates = [3e-4, 1e-3, 1e-4, 1e-5]

# # 모든 조합에 대해 실행 (pretrain_lr, fine_tune_lr, unfreeze_lr)
# for pretrain_lr in lr_candidates:
#     for fine_tune_lr in lr_candidates:
#         for unfreeze_lr in lr_candidates:
#             # 결과 저장 디렉터리 형식: [pretrain_lr]_[fine_tune_lr]_[unfreeze_lr]
#             lr_dir = f"{pretrain_lr}_{fine_tune_lr}_{unfreeze_lr}"
#             RESULT_DIR = f"/home/bcml1/sigenv/_5주차_eeg_SEEDIV/DE+PSD_up2_inter_LR_check/{lr_dir}"
#             print(f"\n=== Running experiment with pretrain_lr={pretrain_lr}, fine_tune_lr={fine_tune_lr}, unfreeze_lr={unfreeze_lr} ===")
#             inter_subject_cv_training(base_dirs, RESULT_DIR, epochs=150, batch_size=16, target_subjects=TARGET_SUBJECTS,
#                                       pretrain_lr=pretrain_lr, fine_tune_lr=fine_tune_lr, unfreeze_lr=unfreeze_lr)
            
# 실행 카운터: 첫 17개 조합은 건너뜁니다.
combination_count = 0
...
for pretrain_lr in lr_candidates:
    for fine_tune_lr in lr_candidates:
        for unfreeze_lr in lr_candidates:
            combination_count += 1
            if combination_count < 53:
                continue  # 53번째 조합부터 실행
            # 결과 저장 디렉터리 형식: [pretrain_lr]_[fine_tune_lr]_[unfreeze_lr]
            lr_dir = f"{pretrain_lr}_{fine_tune_lr}_{unfreeze_lr}"
            RESULT_DIR = f"/home/bcml1/sigenv/_5주차_eeg_SEEDIV/DE+PSD_up2_inter_LR_check/{lr_dir}"
            print(f"\n=== Running experiment {combination_count} with pretrain_lr={pretrain_lr}, fine_tune_lr={fine_tune_lr}, unfreeze_lr={unfreeze_lr} ===")
            inter_subject_cv_training(base_dirs, RESULT_DIR, epochs=150, batch_size=16, target_subjects=TARGET_SUBJECTS,
                                      pretrain_lr=pretrain_lr, fine_tune_lr=fine_tune_lr, unfreeze_lr=unfreeze_lr)
