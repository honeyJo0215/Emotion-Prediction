...
# =============================================================================
# GPU 설정 최적화: 메모리 제한 대신 메모리 growth 활성화
def configure_gpu():
...
# =============================================================================
# (옵션) 혼합 정밀도 사용: GPU가 지원한다면 학습 속도 향상에 도움될 수 있음
...
# =============================================================================
# DE feature 2D mapping 함수 (최적화 버전)
def map_channels_to_2d(de_segment):
    """
    de_segment: numpy array, shape (62, 4)
    패딩 후 (8,8,4) 형태로 재구성.
    """
    padded = np.pad(de_segment, ((0, 64 - de_segment.shape[0]), (0, 0)), mode='constant')
    return padded.reshape(8, 8, de_segment.shape[1]).astype(np.float32)
...
# =============================================================================
# 파일 목록을 생성하고 train/val/test로 분할
...
# =============================================================================
# tf.data.Dataset.from_generator를 이용하여 lazy dataset 생성 및 cache 적용
...
BATCH_SIZE = 16  # 배치 사이즈도 메모리 상황에 맞게 조정
