...
# =============================================================================
# Generator 함수: 주어진 파일 목록(task_list)을 순회하며 각 파일의 샘플을 yield
def data_generator(task_list):
    for de_file_path, raw_file_path, file_name in task_list:
        try:
            # 메모리 맵을 사용하여 데이터를 읽으면 전체 데이터를 메모리에 로드하지 않음
...
# =============================================================================
# tf.data.Dataset.from_generator를 이용하여 lazy dataset 생성
# 출력 형상과 타입을 명시합니다.
...
cm = confusion_matrix([], [])  # 기본값 초기화
# 실제 라벨은 test_dataset에서 extract (예시로 전체 dataset을 np.concatenate하여 비교)
y_true = []
