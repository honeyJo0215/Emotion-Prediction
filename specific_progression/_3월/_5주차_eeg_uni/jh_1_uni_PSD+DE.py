...
from tensorflow.keras.layers import Conv3D, Dense, LayerNormalization, Dropout, Flatten, Input, TimeDistributed, Lambda
...
RESULT_DIR = "/home/bcml1/sigenv/_5주차_eeg_uni/uni_1_4_PSD+DE"
...
class TransformerEncoderLayer(tf.keras.layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        # Transformer Encoder는 입력과 동일한 shape를 출력합니다.
        return input_shape
...
# CNN branch를 정의하는 함수 (TimeDistributed로 적용)
def cnn_branch(x, d_model):
    x = PreprocessLayer()(x)
...
class CNNBranchLayer(tf.keras.layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        # input_shape: (batch, 4, 8, 8, 1) 혹은 (4, 8, 8, 1)일 수 있음.
        # 반환 shape는 (batch, d_model) 또는 (d_model,) 입니다.
...
def build_model(input_shape=(2, 4, 8, 8, 1), n_layers=2, n_heads=4, d_ff=512, p_drop=0.3, d_model=64):
    ...
    # 모달리티 분리: Lambda 레이어를 사용하여 인덱싱 (tf.split 대신)
    de_input = Lambda(lambda x: x[:, 0, ...], name="DE_input")(inputs)   # shape: (batch, 4, 8, 8, 1)
    psd_input = Lambda(lambda x: x[:, 1, ...], name="PSD_input")(inputs)  # shape: (batch, 4, 8, 8, 1)
    
    # 각 모달리티 별 별도 CNN branch (가중치가 독립적임)
    cnn_de = CNNBranchLayer(d_model, name="CNN_DE")
    cnn_psd = CNNBranchLayer(d_model, name="CNN_PSD")
    
    feature_de = cnn_de(de_input)    # shape: (batch, d_model)
    feature_psd = cnn_psd(psd_input)   # shape: (batch, d_model)
    
    # 두 feature를 시퀀스로 구성: Transformer의 입력은 (batch, sequence_length, d_model)
    # 여기서 sequence_length=2 (첫번째: DE, 두번째: PSD)
    ...
    # 시퀀스 차원을 Flatten 하여 최종 feature vector (batch, 2*d_model)
    x = Flatten()(x)
...
# =============================================================================
# Callback: 각 stage 별 learning rate 변경 (pretraining, fine_tuning, unfreeze)
class StageLearningRateScheduler(tf.keras.callbacks.Callback):
    def __init__(self, pretraining_lr, fine_tuning_lr, unfreeze_lr, stage_epochs):
        """
        stage_epochs: tuple, (epoch_switch_to_finetune, epoch_switch_to_unfreeze)
        """
        super(StageLearningRateScheduler, self).__init__()
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if epoch < self.stage_epochs[0]:
            new_lr = self.pretraining_lr
            stage = "pretraining"
        elif epoch < self.stage_epochs[1]:
            new_lr = self.fine_tuning_lr
            stage = "fine_tuning"
        else:
            new_lr = self.unfreeze_lr
            stage = "unfreeze"
        # 옵티마이저의 learning_rate가 tf.Variable이면 assign, 그렇지 않으면 직접 설정
        lr = self.model.optimizer.learning_rate
        if isinstance(lr, tf.Variable):
            lr.assign(new_lr)
        else:
            self.model.optimizer.learning_rate = new_lr
        print(f"Epoch {epoch+1}: setting learning rate to {new_lr} for stage {stage}")

# Callback: val accuracy가 개선될 때마다 모델 저장 후 저장된 모델 불러오기
class SaveAndLoadBestModel(tf.keras.callbacks.Callback):
    def __init__(self, filepath):
        super(SaveAndLoadBestModel, self).__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current_val_acc is not None and current_val_acc > self.best_val_acc:
            ...
            print(f"Epoch {epoch+1}: val_accuracy improved to {current_val_acc:.4f}, model saved to {self.filepath}")
            # 불러와서 추가 학습 (weights 갱신)
            self.model.load_weights(self.filepath)
            print(f"Epoch {epoch+1}: Best model weights loaded for further training.")
...
# 하이퍼파라미터: 세 stage의 learning rate 지정
pretraining_lr = 1e-4
fine_tuning_lr = 1e-5
unfreeze_lr = 1e-6
# stage 변경 epoch (예: 50 epoch에서 fine tuning, 100 epoch에서 unfreeze)
stage_epochs = (50, 100)
...
for subj in unique_subjects:
    ...
    # 모델 생성 및 컴파일
    # 이제 input_shape은 (2,4,8,8,1)
    model = build_model(input_shape=(2,4,8,8,1))
    optimizer = tf.keras.optimizers.Adam(learning_rate=pretraining_lr)
    ...
    callbacks = [
        ...
        StageLearningRateScheduler(pretraining_lr, fine_tuning_lr, unfreeze_lr, stage_epochs),
        SaveAndLoadBestModel(filepath=os.path.join(subj_folder, "best_model_eeg_cnn_transformer.keras"))
    ]
