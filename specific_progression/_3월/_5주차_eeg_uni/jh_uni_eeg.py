...
# =============================================================================
# DE feature와 Raw 데이터 로드 및 매칭 함수
    ...
    for sub in de_subfolders:
        ...
        for file_name in os.listdir(de_folder):
            ...
            try:
                ...
                continue
            # 파일명에서 라벨 추출 (예: "xxxx_label_X.npy")
            try:
            ...
            # T가 예상보다 1 큰 경우 (즉, n_seconds*200 + 1)라면 마지막 샘플 제거
            if T == expected_T + 1:
            ...
            for t in range(n_seconds):
                ...
                X_de.append(mapped_img)
                
                # Raw 데이터 branch: t초에 해당하는 200 sample 추출 → (62,200,4)
                start_idx = t * 200
                end_idx = start_idx + 200
                raw_segment = raw_data[:, start_idx:end_idx, :]
                X_raw.append(raw_segment)
                Y.append(label)
            print(f"Loaded {n_seconds} segments from\n  {de_file_path}\n  {raw_file_path} with label {label}.")
    X_de = np.array(X_de)
...
def build_multimodal_model(de_input_shape=(4,8,8,1), raw_input_shape=(62,200,4),
    ...
    # 두 branch의 특징 벡터를 Concatenate
    combined = Concatenate()([x_de, x_raw])
    # concatenate된 feature를 d_model 차원으로 projection (선택적)
...
# =============================================================================
# 데이터 분할 및 학습
# 전체 데이터를 train/validation/test (예: train 60%, val 20%, test 20%)로 분할
