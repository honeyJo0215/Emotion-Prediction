...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_4월/_diffusion/wo_diff_CNN_diff2"
...
# =============================================================================
# TransposeLayer: 사용자 정의 layer (필요시 사용)
class TransposeLayer(layers.Layer):
    ...
    def compute_output_shape(self, input_shape):
        output_shape = [input_shape[i] for i in self.perm]
        return tuple(output_shape)

# =============================================================================
# 기존의 가우시안 노이즈 추가 함수는 더 이상 사용하지 않음
# def add_diffusion_noise(x, stddev=0.05):
#     noise = tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=stddev)
#     return x + noise

# =============================================================================
# Frequency branch CNN block (diffusion 제거)
# 입력 shape: (200,8,4)
def build_freq_branch(input_shape=(200,8,4)):
...
# =============================================================================
# Channel branch CNN block (diffusion 제거)
# 입력 shape: (200,4,8)
def build_chan_branch(input_shape=(200,4,8)):
    inp = layers.Input(shape=input_shape)
    x = layers.Conv2D(16, (3,3), activation='relu', padding='same')(inp)
    x = layers.MaxPooling2D((2,2))(x)
    x = layers.Conv2D(32, (3,3), activation='relu', padding='same')(x)
...
# =============================================================================
# 전체 모델 구성: 주파수와 채널 branch를 결합하는 모델 (diffusion 제거)
def build_separated_model(num_classes=4):
    # 입력 shape: (4,200,8)
    ...
    freq_branch = build_freq_branch(input_shape=(200,8,4))
    ...
    chan_branch = build_chan_branch(input_shape=(200,4,8))
...
if __name__ == "__main__":
    ...
    for subj in unique_subjects:
        # 여기서는 예시로 subject 번호가 2 이상 10 미만인 경우만 진행
        ...
        model = build_separated_model(num_classes=4)
