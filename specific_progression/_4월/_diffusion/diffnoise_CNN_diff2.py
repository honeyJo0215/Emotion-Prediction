...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_4월/_diffusion/rreal_diff_CNN_diff2"
...
def build_freq_branch_with_diffusion(input_shape=(200,8,4), noise_std=0.05):
    ...
    # ----- Pre-CNN block -----
    # diffusion noise를 적용한 입력과 원본 입력을 residual connection
    noisy_input = layers.Lambda(lambda x: add_diffusion_noise(x, stddev=noise_std))(inp)
    pre_processed = layers.Add()([noisy_input, inp])
...
def build_chan_branch_with_diffusion(input_shape=(200,4,8), noise_std=0.05):
    ...
    # ----- Pre-CNN block -----
    # diffusion noise 적용 후 원본과 residual connection 구성
    noisy_input = layers.Lambda(lambda x: add_diffusion_noise(x, stddev=noise_std))(inp)
    pre_processed = layers.Add()([noisy_input, inp])
...
# =============================================================================
# 전체 모델 구성: 주파수와 채널 branch를 결합하는 모델 (pre-CNN에는 diffusion noise와 residual connection, post-CNN은 DiffuSSM 적용)
