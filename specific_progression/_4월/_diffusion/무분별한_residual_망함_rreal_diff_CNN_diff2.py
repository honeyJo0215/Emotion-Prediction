...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_4월/_diffusion/diff_CNN_diff2"
...
# =============================================================================
# DiffuSSMLayer: Dense + LayerNormalization을 통해 노이즈 제거(denoising) 수행
...
# =============================================================================
# 전체 모델 구성: 원본 및 diffusion 적용 feature들을 결합
def build_model_with_full_diffusion(num_classes=4, noise_std=0.05):
    flat_dim = 4 * 200 * 8  # 6400

    # 입력: (4,200,8)
    input_csp = layers.Input(shape=(4,200,8), name="CSP_Input")
    # Flatten하여 원본 신호 A (6400,)
    flat = layers.Flatten()(input_csp)
    A = flat
    
    # Pre-CNN Diffusion: A에 노이즈 추가 후 DiffuSSMLayer를 적용하여 denoising, residual connection -> B
    noisy_flat = layers.Lambda(lambda x: add_diffusion_noise(x, stddev=noise_std), name="Noisy_Flat")(A)
    diffu1 = DiffuSSMLayer(hidden_dim=flat_dim, name="DiffuSSM_PreCNN")(noisy_flat)
    B = layers.Add(name="PreDiff_Residual")([A, diffu1])
    
    # CNN Branch: 주파수와 채널 branch를 통해 CNN feature C 추출
    # Frequency branch: Permute (batch,4,200,8) -> (batch,200,8,4)
    ...
    # Channel branch: Permute (batch,4,200,8) -> (batch,200,4,8)
    ...
    # CNN feature C: 두 branch의 결과 결합
    C = layers.Concatenate(name="CNN_Features")([freq_feat, chan_feat])  # shape (128,)
    
    # Post-CNN Diffusion: C에 노이즈 추가 후 DiffuSSMLayer를 적용, residual -> D
    noisy_C = layers.Lambda(lambda x: add_diffusion_noise(x, stddev=noise_std), name="Noisy_CNN_Feature")(C)
    diffu2 = DiffuSSMLayer(hidden_dim=128, name="DiffuSSM_PostCNN")(noisy_C)
    D = layers.Add(name="PostDiff_Residual")([C, diffu2])
    
    # 최종 feature 결합: 원본 A, Pre-CNN diffusion B, CNN feature C, Post-CNN diffusion D
    combined_features = layers.Concatenate(name="Combined_All_Features")([A, B, C, D])
    x = layers.Dense(128, activation='relu')(combined_features)
    x = layers.Dropout(0.3)(x)
    x = layers.Dense(64, activation='relu')(x)
    x = layers.Dropout(0.3)(x)
    output = layers.Dense(num_classes, activation='softmax', name="EmotionClassifier")(x)
    
    model = models.Model(inputs=input_csp, outputs=output, name="FullDiffusion_EEG_Model")
    return model
...
if __name__ == "__main__":
    ...
    for subj in unique_subjects:
        ...
        model = build_model_with_full_diffusion(num_classes=4, noise_std=0.05)
