...
# =============================================================================
# 결과 저장 경로 설정
RESULT_DIR = "/home/bcml1/sigenv/_4월/_diffusion/A_diff_CNN2"
...
class DiffuSSMLayer(layers.Layer):
    def __init__(self,
                 model_dim,           # 입력/출력 차원 (flat vector 길이)
                 hidden_dim,          # bottleneck 차원
                 output_units=None,   # 복원할 출력 차원 (flat vector 길이)
                 cond_dim=None,       # optional conditioning dim
                 alpha_init=0.0,      # fusion scale 초기값
                 **kwargs):
        super().__init__(**kwargs)
        self.model_dim   = model_dim
        self.hidden_dim  = hidden_dim
        self.output_units= output_units or model_dim

        # 1) Layer Norm (+ optional conditioning)
        ...
        # 3) 복원용 최종 Dense
        self.out_dense = layers.Dense(self.output_units, activation=None, name="restore_dense")
    ...
    def call(self, x, cond=None, training=False):
        ...
        # 2) Hourglass down → SSM → Hourglass up
        h = self.hg_down(x_ln)
        h = self.ssm(h, training=training)
        h = self.hg_up(h)

        # 3) Residual fusion
        fused = x + self.alpha * h

        # 4) 복원 Dense
        return self.out_dense(fused)
...
def build_freq_branch_with_diffusion(input_shape=(200,8,4), noise_std=0.05):
    ...
    # ----- Pre-CNN: diffusion noise 추가 후 DiffuSSM으로 복원 -----
    # noise → flatten
    noisy = layers.Lambda(lambda x: add_diffusion_noise(x, stddev=noise_std))(inp)
    flat  = layers.Flatten()(noisy)
    # DiffuSSM 복원: model_dim = flat 길이, output_units = flat 길이
    flat_dim = np.prod(input_shape)
    restored = DiffuSSMLayer(
        model_dim=flat_dim,
        hidden_dim=256,
        output_units=flat_dim
    )(flat)

    # 다시 원래 shape 으로
    restored_signal = layers.Reshape(input_shape)(restored)

    # ----- CNN block: 복원된 신호로부터 feature 추출 -----
    x = layers.Conv2D(16, (3,3), activation='relu', padding='same')(restored_signal)
    x = layers.MaxPooling2D((2,2))(x)
    x = layers.Conv2D(64, (3,3), activation='relu', padding='same')(x)
    x = layers.GlobalAveragePooling2D()(x)  # 출력 shape: (64,)
    freq_feat = x
    
    branch = models.Model(inputs=inp, outputs=freq_feat, name="FreqBranchDiff")
    return branch
...
def build_chan_branch_with_diffusion(input_shape=(200,4,8), noise_std=0.05):
    ...
    branch = models.Model(inputs=inp, outputs=chan_feat, name="ChanBranchDiff")
    return branch

# =============================================================================
# 전체 모델 구성: 주파수와 채널 branch를 결합하여 최종 분류 수행
