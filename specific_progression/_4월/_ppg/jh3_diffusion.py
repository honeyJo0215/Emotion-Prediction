...
# =============================================================================
# [Data Augmentation Functions] - numpy 기반, 데이터 로드 시 적용
...
# =============================================================================
# 각 분할(Train, Val, Test) 내에서 증강 수행하는 함수
def augment_split(X, y, augmentation_funcs):
    augmented_X = []
    augmented_y = []
...
# =============================================================================
# 4. 1s 데이터를 non-overlapping하게 그룹화하여 10s 샘플로 변환 (예: 60s면 6개 샘플)
...
def train_model_for_subject(subject_id, X, y, num_classes=4, epochs=3000, batch_size=64):
    ...
    # 각 분할 내에서 증강 수행 (각 샘플당 10가지)
    X_train_aug, y_train_aug = augment_split(X_train, y_train, augmentation_functions)
    X_val_aug, y_val_aug     = augment_split(X_val, y_val, augmentation_functions)
    X_test_aug, y_test_aug   = augment_split(X_test, y_test, augmentation_functions)
    
    print(f"Subject {subject_id}: Train {X_train_aug.shape[0]} samples, Val {X_val_aug.shape[0]} samples, Test {X_test_aug.shape[0]} samples.")
    
    result_dir = os.path.join(result_dir_base, subject_id)
    os.makedirs(result_dir, exist_ok=True)

    input_length = X.shape[1]  # 예: 1280 for 10s
    ...
    model.summary()
    history = model.fit(X_train_aug, y_train_aug, epochs=epochs, batch_size=batch_size,
                        validation_data=(X_val_aug, y_val_aug), verbose=1)
    ...
    y_pred = np.argmax(model.predict(X_test_aug), axis=1)
    eval_path = os.path.join(result_dir, f"{subject_id}_evaluation.txt")
    with open(eval_path, 'w') as f:
        f.write(classification_report(y_test_aug, y_pred))
        f.write("\nConfusion Matrix:\n")
        f.write(str(confusion_matrix(y_test_aug, y_pred)))
    
    plt.figure(figsize=(6, 5))
    sns.heatmap(confusion_matrix(y_test_aug, y_pred), annot=True, fmt="d", cmap="Blues")
...
# =============================================================================
# 6. 메인: 모드 선택 후 전체 피실험자에 대해 intra-subject cross validation 실행
if __name__ == "__main__":
    ...
    if mode == '1s':
        ...
        result_dir_base = '/home/bcml1/sigenv/_4월/_ppg/2rx_diffnoise_CNN_diff2_intra_1s'
    elif mode == '10s':
        # 10s 모드에서는 1s 데이터를 이어붙여 사용 (overlap 없이)
        data_dir = '/home/bcml1/2025_EMOTION/DEAP_PPG_1s'
        subject_data = load_data_1s(data_dir, ch_choice_pair1=1, ch_choice_pair2=3)
        subject_data = group_1s_to_10s(subject_data, segment_length=10)  # 10초 샘플 생성 (예: 60s -> 6개)
        result_dir_base = '/home/bcml1/sigenv/_4월/_ppg/2rx_diffnoise_CNN_diff2_intra_10s'
    else:
