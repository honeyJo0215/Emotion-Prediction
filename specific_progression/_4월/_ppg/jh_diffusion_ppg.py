...
# output_units 인자가 주어지면 마지막 Dense layer의 unit 수로 사용 (flatten한 입력을 원래 shape로 복원할 때 사용)
...
class EMCNN(tf.keras.Model):
    def __init__(self, num_classes=4, smoothing_kernel_size=2, downsample_factor=2,
        ...
        self.input_length = input_length  # 예: 128 (1s) 또는 1280 (10s)
        ...
        # downsampling branch의 경우 입력 길이는 downsampled 값
        ...
        # 각 브랜치에서 노이즈가 적용된 입력을 복원하기 위한 DiffuSSMLayer (pre-feature extractor)
    ...
    def call(self, inputs, training=False):
        ...
        각 브랜치에서 입력에 노이즈를 적용한 후 DiffuSSMLayer로 복원하고 feature extractor에 전달합니다.
        """
        batch_size = tf.shape(inputs)[0]
        L = self.input_length

        # Identity branch:
        noisy_identity = add_diffusion_noise(inputs, stddev=self.internal_noise_std)
        flat_identity = tf.reshape(noisy_identity, [batch_size, L * 3])  # 명시적으로 L*3 사용
        ...
        # Smoothing branch:
        smooth_input = self.smoothing_conv(inputs)  # (batch, L, 3)
        noisy_smooth = add_diffusion_noise(smooth_input, stddev=self.internal_noise_std)
        ...
        # Downsampling branch:
        down_input = inputs[:, ::self.downsample_factor, :]  # (batch, L/downsample_factor, 3)
        noisy_down = add_diffusion_noise(down_input, stddev=self.internal_noise_std)
...
def load_data_1s(data_dir, ch_choice_pair1=1, ch_choice_pair2=3):
    ...
    파일 shape: (60, 5, 128)  
    선택: 첫 번째 채널(원본)과 두번째, 세번째 채널 중 선택  
    최종 shape: (60, 128, 3)
    """
    subject_data = {}
...
if __name__ == "__main__":
    ...
    if mode == '1s':
        ...
        result_dir_base = '/home/bcml1/sigenv/_4월/_ppg/diff_CNN2_intra_1s'
    elif mode == '10s':
        ...
        result_dir_base = '/home/bcml1/sigenv/_4월/_ppg/diff_CNN2_intra_10s'
    else:
    ...
    for subject_id in sorted(subject_data.keys(), key=lambda x: int(x[1:])):
        ...
        train_model_for_subject(subject_id, X, y, num_classes=4, epochs=3000, batch_size=64)
