...
class EMCNN(tf.keras.Model):
    def __init__(self, num_classes=4, smoothing_kernel_size=2, downsample_factor=2, input_length=None, num_channels=5):
        ...
        # 각 브랜치별 Feature Extractor (각 branch는 5채널 입력을 그대로 사용)
...
# -----------------------------
# 3. 학습 및 평가 (intra-subject cross validation: 5-Fold)
# -----------------------------
def train_model_for_subject(subject_id, X, y, num_classes=4, epochs=50, batch_size=8,
    ...
    """
    subject_id: 예, 's01'
    X: 입력 데이터, shape = (total_samples, L, num_channels)
    y: 원래의 정수 라벨, shape = (total_samples,)
    intra-subject cross validation을 수행하여, 각 fold별로 모델을 학습 및 평가합니다.
    각 fold의 classification report와 confusion matrix, 그리고 모든 fold를 통합한 global report를 저장합니다.
    저장 폴더 이름은 result_dir_base에 mode (_1s 또는 _10s)가 포함됩니다.
    """
