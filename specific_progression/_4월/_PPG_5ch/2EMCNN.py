...
# 설정: 결과 저장 경로
# -------------------------------------------------------------------------
RESULT_DIR_BASE = '/home/bcml1/sigenv/_4월/_PPG_5ch/EMCNN'
...
def build_multibranch_model(seq_len=128, hrv_len=128, cwt_shape=(32,4,1)):
    ...
    def ppg_branch(inp):
        ...
        x = MaxPooling1D(2)(x)
        return Flatten()(x)
    out1 = ppg_branch(inp_o)
    out2 = ppg_branch(inp_s)
    out3 = ppg_branch(inp_d)
    
    h = Dense(64,activation='relu')(inp_h)
    h = Dropout(0.3)(h)
    out4 = Dense(32,activation='relu')(h)
    ...
    merged = concatenate([out1,out2,out3,out4,out5])
