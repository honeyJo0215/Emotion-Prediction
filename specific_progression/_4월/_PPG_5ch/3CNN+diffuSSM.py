# 이러한 bidirectional 을 사용한 diffuSSM을 사용하여 
# basic CNN 모델과 diffuSSM을 결합한 모델의 성능을 확인해보자. 이유: EMCNN이 정말 좋은 성능을 가지는지 파악하기 위해서
# 만약 기본적으로 쓸때 기본적인 CNN 보다 더 뛰어나더라도 diffuSSM을 사용할 때는 성능이 달라질 수 있으니 그것도 확인하기 위해서.
...
# 설정: 결과 저장 경로
# -------------------------------------------------------------------------
RESULT_DIR_BASE = '/home/bcml1/sigenv/_4월/_PPG_5ch/CNN+diffuSSM'
...
def build_separated_model_with_diffusion(
    seq_len=128,
    hrv_len=128,
    cwt_shape=(32,4,1),
    num_classes=4,
    ...
    """
    5채널 입력(orig, smooth, down, hrv, cwt)을 받아
    각 브랜치별로 CNN → Flatten → DiffuSSM → 결합 → FC → 출력하는 모델을 생성.
    """

    # 1) 입력 정의
    inp_o = Input((seq_len,1),    name="orig")    # 원본 PPG
    inp_s = Input((seq_len,1),    name="smooth")  # smoothing
    inp_d = Input((seq_len,1),    name="down")    # downsampling
    inp_h = Input((hrv_len,),     name="hrv")     # HRV 시리즈
    inp_c = Input(cwt_shape,      name="cwt")     # CWT 맵

    # 2) PPG용 1D-CNN + DiffuSSM 브랜치 정의
    ...
    # 4) CWT 브랜치: 2D-CNN → Flatten → DiffuSSM
    ...
    model = Model(
        ...
        outputs=out,
        name="Separated_PPG_with_DiffuSSM"
    )
    return model
