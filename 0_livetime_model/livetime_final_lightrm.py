...
def remove_glare(img, thresh_val=240, dilate_iter=2, inpaint_radius=5):
    """
    간단한 스펙큘러 하이라이트(빛 반사) 마스크 생성 후 인페인트로 제거.
    - img: BGR 이미지
    - thresh_val: 흰색 스펙큘러 픽셀 검출 임계값 (0–255)
    - dilate_iter: 마스크 확장 정도
    - inpaint_radius: 인페인트 반경
    """
...
def your_extract_eog_features(frames, fps=30.0, window_secs=1.0):
    ...
    frames = [remove_glare(f, thresh_val=240, dilate_iter=2, inpaint_radius=5)
              for f in frames]
