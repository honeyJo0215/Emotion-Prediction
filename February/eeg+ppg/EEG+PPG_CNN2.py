...
SAVE_PATH    = "/home/bcml1/sigenv/_3주차_eeg+pps_CNN/m_result2"
...
def grid_search_lambda(train_data, val_data, alpha_values):
    ...
    print(f"λ value searched:  (Grid Search): {best_lambdas}")
    return best_lambdas  
...
def load_multimodal_data(subject):
    ...
    if len(eeg_files) == 0 or len(ppg_files) == 0:
        raise FileNotFoundError(f"{subject} Error: Can't find subject.")
    if len(eeg_files) != len(ppg_files):
        print("WANING: EEG-PPG mismatch.")
    ...
    eeg_array = np.concatenate(eeg_segments, axis=0)  # (seg*40, 32, 128)
    ppg_array = np.concatenate(ppg_segments, axis=0)  # (seg*40, 128)
    ...
    if not os.path.exists(label_file):
        raise FileNotFoundError(f"missed label file: {label_file}")
...
def train_multimodal(use_bayesian=True, max_iterations=20):
    ...
    for subject in subjects:
        ...
        try:
            ...
            print(f"{subject} ERROR: load failed {e}")
            continue
        ...
        print(f"lambda setted: λ1={lambda1}, λ2={lambda2}, λ3={lambda3}")
