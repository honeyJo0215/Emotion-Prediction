...
from tensorflow.keras.layers import (Conv3D, Conv2D, DepthwiseConv2D, Dense, LayerNormalization, 
                                     Dropout, Flatten, Input, MaxPooling2D, Concatenate, Reshape, 
...
# Directory
RESULT_DIR = "/home/bcml1/sigenv/_5주차_eeg_uni/uni_6"
...
class CustomModelCheckpointAndLrUpdater(tf.keras.callbacks.Callback):
    def __init__(self, checkpoint_dir, monitor="val_accuracy", factor=0.5, verbose=1):
        super(CustomModelCheckpointAndLrUpdater, self).__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        checkpoint_path = os.path.join(self.checkpoint_dir, f"epoch_{epoch+1:02d}_val_acc_{current:.4f}.keras")
        ...
        if current > self.best:
            ...
            if self.verbose:
                ...
                print(f"Updating learning rate from {old_lr:.6f} to {new_lr:.6f}.")
            self.best = current
...
def build_raw_cnn_branch(input_shape=(62,200,4), d_model=64):
    ...
    x = MaxPooling2D(pool_size=(2,2))(x)
    # Flatten spatial dimensions via GlobalAveragePooling2D to obtain one feature map per channel
    x = tf.keras.layers.GlobalAveragePooling2D()(x)  # (batch, channels)
    # Project to d_model dimension
    x = Dense(d_model, activation="relu")(x)  # (batch, d_model)
    # Expand dims to create a token sequence of length 1
    x = Lambda(lambda z: tf.expand_dims(z, axis=1))(x)  # (batch, 1, d_model)
...
def build_multimodal_model(de_input_shape=(4,8,8,1), raw_input_shape=(62,200,4),
    ...
    # Global average pooling over tokens to get single token representation
    x_de = GlobalAveragePooling1D()(x_de)  # (batch, d_model)
    # Expand dims to create token sequence of length 1
    token_de = Lambda(lambda z: tf.expand_dims(z, axis=1))(x_de)  # (batch, 1, d_model)
    
    # Raw branch
    raw_branch = build_raw_cnn_branch(input_shape=raw_input_shape, d_model=d_model)
    # raw_branch already outputs shape (batch, 1, d_model)
    token_raw = raw_branch.output  # (batch, 1, d_model)
    
    # (batch, 2, d_model)
    combined_tokens = Concatenate(axis=1)([token_de, token_raw])
    ...
    # Global average pooling over the 2 tokens
    x = GlobalAveragePooling1D()(x)
    outputs = Dense(num_classes, activation="softmax")(x)
    
    model = Model(inputs=[de_input, raw_branch.input], outputs=outputs, name="multimodal_model")
    return model
...
callbacks = [
    ...
    CustomModelCheckpointAndLrUpdater(checkpoint_dir=RESULT_DIR, monitor="val_accuracy", factor=0.5, verbose=1)
]
